import{n as e,r as t,t as n}from"./index-BbP2JjP3.js";var r=`#version 300 es
...
`,w={maxColorCount:10},T=`#version 300 es
precision mediump float;
...
`,E={maxColorCount:10,maxNoiseIterations:8},ee=`#version 300 es
precision mediump float;
...
`,ne={maxColorCount:10},re=`#version 300 es
precision mediump float;
...
`,ae={circle:0,diamond:1,square:2,triangle:3},D={maxColorCount:10},oe=`#version 300 es
precision mediump float;
...
void main() {
  ...
  vec4 gradient = u_colors[0];
  gradient.rgb *= gradient.a;
  for (int i = 1; i < ${D.maxColorCount}; i++) {
    if (i >= int(u_colorsCount)) break;
...
`,se={maxColorCount:8,maxBallsCount:20},ce=`#version 300 es
precision mediump float;
...
void main() {
  ...
  for (int i = 0; i < ${se.maxBallsCount}; i++) {
    if (i >= int(ceil(u_count))) break;

    float idxFract = float(i) / float(${se.maxBallsCount});
...
`,ue={maxColorCount:5},de=`#version 300 es
precision mediump float;
...
`,pe={maxColorCount:10},me=`#version 300 es
precision mediump float;
...
`,he={checks:0,stripes:1,edge:2},ge={maxColorCount:5},_e=`#version 300 es
precision mediump float;
...
`,ye={maxColorCount:10},be=`#version 300 es
precision mediump float;
...
`,Se={simplex:1,warp:2,dots:3,wave:4,ripple:5,swirl:6,sphere:7},Ce={random:1,"2x2":2,"4x4":3,"8x8":4},we={maxColorCount:7},Te=`#version 300 es
...
void main() {
  ...
  vec4 gradient = u_colors[0];
  gradient.rgb *= gradient.a;
  for (int i = 1; i < ${we.maxColorCount}; i++) {
    if (i > cntStop) break;
...
`,Ee={wave:1,dots:2,truchet:3,corners:4,ripple:5,blob:6,sphere:7},De={maxColorCount:5,maxSpots:4},Oe=`#version 300 es
precision lowp float;
...
void main() {
  ...
  fragColor = vec4(color, opacity);
}`,ke={auto:0,square:1},Ae={maxColorCount:7},je=`#version 300 es
precision lowp float;
...
`,Me={maxColorCount:10},Ne=`#version 300 es
precision mediump float;
...
`,Pe={maxColorCount:10},Fe=`#version 300 es
precision mediump float;
...
`,ze={lines:1,linesIrregular:2,wave:3,zigzag:4,pattern:5},Be={prism:1,lens:2,contour:3,cascade:4,flat:5},Ve=`#version 300 es
precision mediump float;
...
`,He={maxSamples:50},Ue=`#version 300 es
precision mediump float;
...
`,We={maxColorCount:10},Ge=`#version 300 es
...
`;function Ke(e){let t=document.createElement(`canvas`),n=1e3;return new Promise((r,i)=>{let a=new Image;a.crossOrigin=`anonymous`,a.addEventListener(`load`,()=>{(typeof e==`string`?e.endsWith(`.svg`):e.type===`image/svg+xml`)&&(a.width=n,a.height=n);let o=a.naturalWidth/a.naturalHeight,s=n,c=n;o>1?c=Math.floor(n/o):s=Math.floor(n*o),t.width=s+2*375,t.height=c+2*375;let l=t.getContext(`2d`,{willReadFrequently:!0});if(!l)throw Error(`Failed to get canvas 2d context`);l.fillStyle=`white`,l.fillRect(0,0,t.width,t.height),l.drawImage(a,375,375,s,c);let{width:u,height:d}=t,f=l.getImageData(0,0,u,d).data,p=u*d,m=new Uint8ClampedArray(p);for(let e=0;e<p;e++){let t=e*4,n=f[t]??0,r=f[t+1]??0,i=f[t+2]??0;m[e]=.299*n+.587*r+.114*i|0}let h=Je(m,u,d,150,3),g=Je(m,u,d,18,3),_=Je(m,u,d,5,1),v=l.createImageData(u,d),y=v.data;for(let e=0;e<p;e++){let t=e*4;y[t]=_[e]??0,y[t+1]=h[e]??0,y[t+2]=g[e]??0,y[t+3]=255}l.putImageData(v,0,0),t.toBlob(e=>{if(!e){i(Error(`Failed to create PNG blob`));return}r({blob:e})},`image/png`)}),a.addEventListener(`error`,()=>{i(Error(`Failed to load image`))}),a.src=typeof e==`string`?e:URL.createObjectURL(e)})}function qe(e,t,n,r){if(r<=0)return e.slice();let i=new Uint8ClampedArray(t*n),a=new Uint32Array(t*n);for(let r=0;r<n;r++){let n=0;for(let i=0;i<t;i++){let o=r*t+i,s=e[o]??0;n+=s,a[o]=n+(r>0?a[o-t]??0:0)}}for(let e=0;e<n;e++){let o=Math.max(0,e-r),s=Math.min(n-1,e+r);for(let n=0;n<t;n++){let c=Math.max(0,n-r),l=Math.min(t-1,n+r),u=s*t+l,d=s*t+(c-1),f=(o-1)*t+l,p=(o-1)*t+(c-1),m=a[u]??0,h=c>0?a[d]??0:0,g=o>0?a[f]??0:0,_=c>0&&o>0?a[p]??0:0,v=m-h-g+_,y=(l-c+1)*(s-o+1);i[e*t+n]=Math.round(v/y)}}return i}function Je(e,t,n,r,i){if(r<=0||i<=1)return qe(e,t,n,r);let a=e,o=e;for(let e=0;e<i;e++)o=qe(a,t,n,r),a=o;return o}var Ye=`#version 300 es
precision mediump float;
...
`,Xe={measurePerformance:!1,workingSize:512,iterations:40};function Ze(e){let t=document.createElement(`canvas`),n=t.getContext(`2d`),r=typeof e==`string`&&e.startsWith(`blob:`);return new Promise((i,a)=>{if(!e||!n){a(Error(`Invalid file or canvas context`));return}let o=r&&fetch(e).then(e=>e.headers.get(`Content-Type`)),s=new Image;s.crossOrigin=`anonymous`;let c=performance.now();s.onload=async()=>{let r,l=await o;r=l?l===`image/svg+xml`:typeof e==`string`?e.endsWith(`.svg`)||e.startsWith(`data:image/svg+xml`):e.type===`image/svg+xml`;let u=s.width||s.naturalWidth,d=s.height||s.naturalHeight;if(r){let e=4096,t=u/d;u>d?(u=e,d=e/t):(d=e,u=e*t),s.width=u,s.height=d}let f=Math.min(u,d),p=Xe.workingSize/f,m=Math.round(u*p),h=Math.round(d*p);Xe.measurePerformance&&(console.log(`[Processing Mode]`),console.log(`  Original: ${u}\xD7${d}`),console.log(`  Working: ${m}\xD7${h} (${(p*100).toFixed(1)}% scale)`),p<1&&console.log(`  Speedup: ~${Math.round(1/(p*p))}\xD7`)),t.width=u,t.height=d;let g=document.createElement(`canvas`);g.width=m,g.height=h;let _=g.getContext(`2d`);_.drawImage(s,0,0,m,h);let v=performance.now(),y=_.getImageData(0,0,m,h).data,b=new Uint8Array(m*h),x=new Uint8Array(m*h),S=0;for(let e=0,t=0;e<y.length;e+=4,t++){let n=y[e+3]===0?0:1;b[t]=n,S+=n}let C=[],w=[];for(let e=0;e<h;e++)for(let t=0;t<m;t++){let n=e*m+t;if(!b[n])continue;let r=!1;r=t===0||t===m-1||e===0||e===h-1||!b[n-1]||!b[n+1]||!b[n-m]||!b[n+m]||!b[n-m-1]||!b[n-m+1]||!b[n+m-1]||!b[n+m+1],r?(x[n]=1,C.push(n)):w.push(n)}Xe.measurePerformance&&(console.log(`[Mask Building] Time: ${(performance.now()-v).toFixed(2)}ms`),console.log(`  Shape pixels: ${S} / ${m*h} (${(S/(m*h)*100).toFixed(1)}%)`),console.log(`  Interior pixels: ${w.length}`),console.log(`  Boundary pixels: ${C.length}`));let T=Qe(b,x,new Uint32Array(w),new Uint32Array(C),m,h),E=performance.now(),ee=$e(T,b,x,m,h);Xe.measurePerformance&&console.log(`[Poisson Solve] Time: ${(performance.now()-E).toFixed(2)}ms`);let te=0,ne;for(let e=0;e<w.length;e++){let t=w[e];ee[t]>te&&(te=ee[t])}let re=document.createElement(`canvas`);re.width=m,re.height=h;let ie=re.getContext(`2d`),ae=ie.createImageData(m,h);for(let e=0;e<h;e++)for(let t=0;t<m;t++){let n=e*m+t,r=n*4;if(!b[n])ae.data[r]=255,ae.data[r+1]=255,ae.data[r+2]=255,ae.data[r+3]=0;else{let e=255*(1-ee[n]/te);ae.data[r]=e,ae.data[r+1]=e,ae.data[r+2]=e,ae.data[r+3]=255}}ie.putImageData(ae,0,0),n.imageSmoothingEnabled=!0,n.imageSmoothingQuality=`high`,n.drawImage(re,0,0,m,h,0,0,u,d);let D=n.getImageData(0,0,u,d),oe=document.createElement(`canvas`);oe.width=u,oe.height=d;let se=oe.getContext(`2d`);se.drawImage(s,0,0,u,d);let ce=se.getImageData(0,0,u,d);for(let e=0;e<D.data.length;e+=4){let t=ce.data[e+3],n=D.data[e+3];t===0?(D.data[e]=255,D.data[e+1]=0):(D.data[e]=n===0?0:D.data[e],D.data[e+1]=t),D.data[e+2]=255,D.data[e+3]=255}n.putImageData(D,0,0),ne=D,t.toBlob(e=>{if(!e){a(Error(`Failed to create PNG blob`));return}if(Xe.measurePerformance){let e=performance.now()-c;if(console.log(`[Total Processing Time] ${e.toFixed(2)}ms`),p<1){let t=e*(u*d/(m*h))**1.5;console.log(`[Estimated time at full resolution] ~${t.toFixed(0)}ms`),console.log(`[Time saved] ~${(t-e).toFixed(0)}ms (${Math.round(t/e)}\xD7 faster)`)}}i({imageData:ne,pngBlob:e})},`image/png`)},s.onerror=()=>a(Error(`Failed to load image`)),s.src=typeof e==`string`?e:URL.createObjectURL(e)})}function Qe(e,t,n,r,i,a){let o=n.length,s=new Int32Array(o*4);for(let t=0;t<o;t++){let r=n[t],o=r%i,c=Math.floor(r/i);s[t*4+0]=o<i-1&&e[r+1]?r+1:-1,s[t*4+1]=o>0&&e[r-1]?r-1:-1,s[t*4+2]=c>0&&e[r-i]?r-i:-1,s[t*4+3]=c<a-1&&e[r+i]?r+i:-1}return{interiorPixels:n,boundaryPixels:r,pixelCount:o,neighborIndices:s}}function $e(e,t,n,r,i){let a=Xe.iterations,o=.01,s=new Float32Array(r*i),{interiorPixels:c,neighborIndices:l,pixelCount:u}=e,d=performance.now(),f=1.9,p=[],m=[];for(let e=0;e<u;e++){let t=c[e];(t%r+Math.floor(t/r))%2==0?p.push(e):m.push(e)}for(let e=0;e<a;e++){for(let e of p){let t=c[e],n=l[e*4+0],r=l[e*4+1],i=l[e*4+2],a=l[e*4+3],u=0;n>=0&&(u+=s[n]),r>=0&&(u+=s[r]),i>=0&&(u+=s[i]),a>=0&&(u+=s[a]),s[t]=f*((o+u)/4)+(1-f)*s[t]}for(let e of m){let t=c[e],n=l[e*4+0],r=l[e*4+1],i=l[e*4+2],a=l[e*4+3],u=0;n>=0&&(u+=s[n]),r>=0&&(u+=s[r]),i>=0&&(u+=s[i]),a>=0&&(u+=s[a]),s[t]=f*((o+u)/4)+(1-f)*s[t]}}if(Xe.measurePerformance){let e=performance.now()-d;console.log(`[Optimized Poisson Solver (SOR \u03C9=${f})]`),console.log(`  Working size: ${r}\xD7${i}`),console.log(`  Iterations: ${a}`),console.log(`  Time: ${e.toFixed(2)}ms`),console.log(`  Interior pixels processed: ${u}`),console.log(`  Speed: ${(a*u/(e*1e3)).toFixed(2)} Mpixels/sec`)}return s}var et={none:0,circle:1,daisy:2,diamond:3,metaballs:4},tt=`#version 300 es
precision mediump float;
...
`,nt={classic:0,gooey:1,holes:2,soft:3},rt={square:0,hex:1},it=`#version 300 es
precision mediump float;
...
`,at={dots:0,ink:1,sharp:2},ot={maxColorCount:6},st=`#version 300 es
precision mediump float;
...
// Colors
uniform vec4 u_colors[${ot.maxColorCount}];
uniform float u_colorsCount;
uniform vec4 u_colorBack;
...
// Smoke UV setup
  ...
  float smokeMask = 0.;
  for (int i = 1; i < ${ot.maxColorCount+1}; i++) {
    if (i > int(u_colorsCount)) break;
...
`,ct={measurePerformance:!1,workingSize:512,iterations:32};function lt(e){let t=document.createElement(`canvas`),n=t.getContext(`2d`),r=typeof e==`string`&&e.startsWith(`blob:`);return new Promise((i,a)=>{if(!e||!n){a(Error(`Invalid file or canvas context`));return}let o=r&&fetch(e).then(e=>e.headers.get(`Content-Type`)),s=new Image;s.crossOrigin=`anonymous`;let c=performance.now();s.onload=async()=>{let r,l=await o;r=l?l===`image/svg+xml`:typeof e==`string`?e.endsWith(`.svg`)||e.startsWith(`data:image/svg+xml`):e.type===`image/svg+xml`;let u=s.width||s.naturalWidth,d=s.height||s.naturalHeight;if(r){let e=4096,t=u/d;u>d?(u=e,d=e/t):(d=e,u=e*t),s.width=u,s.height=d}let f=Math.min(u,d),p=ct.workingSize/f,m=Math.round(u*p),h=Math.round(d*p);ct.measurePerformance&&(console.log(`[Processing Mode]`),console.log(`  Original: ${u}\xD7${d}`),console.log(`  Working: ${m}\xD7${h} (${(p*100).toFixed(1)}% scale)`),p<1&&console.log(`  Speedup: ~${Math.round(1/(p*p))}\xD7`)),t.width=u,t.height=d;let g=.025,_=Math.ceil(m*g),v=Math.ceil(h*g),y=m-2*_,b=h-2*v,x=document.createElement(`canvas`);x.width=m,x.height=h;let S=x.getContext(`2d`);S.drawImage(s,_,v,y,b);let C=performance.now(),w=S.getImageData(0,0,m,h).data,T=new Uint8Array(m*h),E=new Uint8Array(m*h),ee=0;for(let e=0,t=0;e<w.length;e+=4,t++){let n=w[e+3]===0?0:1;T[t]=n,ee+=n}let te=[],ne=[];for(let e=0;e<h;e++)for(let t=0;t<m;t++){let n=e*m+t;if(!T[n])continue;let r=!1;r=t===0||t===m-1||e===0||e===h-1||!T[n-1]||!T[n+1]||!T[n-m]||!T[n+m]||!T[n-m-1]||!T[n-m+1]||!T[n+m-1]||!T[n+m+1],r?(E[n]=1,te.push(n)):ne.push(n)}ct.measurePerformance&&(console.log(`[Mask Building] Time: ${(performance.now()-C).toFixed(2)}ms`),console.log(`  Shape pixels: ${ee} / ${m*h} (${(ee/(m*h)*100).toFixed(1)}%)`),console.log(`  Interior pixels: ${ne.length}`),console.log(`  Boundary pixels: ${te.length}`));let re=ut(T,E,new Uint32Array(ne),new Uint32Array(te),m,h),ie=performance.now(),ae=dt(re,T,E,m,h);ct.measurePerformance&&console.log(`[Poisson Solve] Time: ${(performance.now()-ie).toFixed(2)}ms`);let D=0,oe;for(let e=0;e<ne.length;e++){let t=ne[e];ae[t]>D&&(D=ae[t])}let se=document.createElement(`canvas`);se.width=m,se.height=h;let ce=se.getContext(`2d`),le=ce.createImageData(m,h);for(let e=0;e<h;e++)for(let t=0;t<m;t++){let n=e*m+t,r=n*4;if(!T[n])le.data[r]=255,le.data[r+1]=255,le.data[r+2]=255,le.data[r+3]=0;else{let e=255*(1-ae[n]/D);le.data[r]=e,le.data[r+1]=e,le.data[r+2]=e,le.data[r+3]=255}}ce.putImageData(le,0,0),n.imageSmoothingEnabled=!0,n.imageSmoothingQuality=`high`,n.drawImage(se,0,0,m,h,0,0,u,d);let ue=n.getImageData(0,0,u,d),de=Math.ceil(u*g),fe=Math.ceil(d*g),pe=document.createElement(`canvas`);pe.width=u,pe.height=d;let me=pe.getContext(`2d`);me.drawImage(s,de,fe,u-2*de,d-2*fe);let he=me.getImageData(0,0,u,d);for(let e=0;e<ue.data.length;e+=4){let t=he.data[e+3],n=ue.data[e+3];t===0?(ue.data[e]=255,ue.data[e+1]=0):(ue.data[e]=n===0?0:ue.data[e],ue.data[e+1]=t),ue.data[e+2]=255,ue.data[e+3]=255}n.putImageData(ue,0,0),oe=ue,t.toBlob(e=>{if(!e){a(Error(`Failed to create PNG blob`));return}if(ct.measurePerformance){let e=performance.now()-c;if(console.log(`[Total Processing Time] ${e.toFixed(2)}ms`),p<1){let t=e*(u*d/(m*h))**1.5;console.log(`[Estimated time at full resolution] ~${t.toFixed(0)}ms`),console.log(`[Time saved] ~${(t-e).toFixed(0)}ms (${Math.round(t/e)}\xD7 faster)`)}}i({imageData:oe,pngBlob:e})},`image/png`)},s.onerror=()=>a(Error(`Failed to load image`)),s.src=typeof e==`string`?e:URL.createObjectURL(e)})}function ut(e,t,n,r,i,a){let o=n.length,s=new Int32Array(o*4);for(let t=0;t<o;t++){let r=n[t],o=r%i,c=Math.floor(r/i);s[t*4+0]=o<i-1&&e[r+1]?r+1:-1,s[t*4+1]=o>0&&e[r-1]?r-1:-1,s[t*4+2]=c>0&&e[r-i]?r-i:-1,s[t*4+3]=c<a-1&&e[r+i]?r+i:-1}return{interiorPixels:n,boundaryPixels:r,pixelCount:o,neighborIndices:s}}function dt(e,t,n,r,i){let a=ct.iterations,o=.01,s=new Float32Array(r*i),{interiorPixels:c,neighborIndices:l,pixelCount:u}=e,d=performance.now(),f=1.9,p=[],m=[];for(let e=0;e<u;e++){let t=c[e];(t%r+Math.floor(t/r))%2==0?p.push(e):m.push(e)}for(let e=0;e<a;e++){for(let e of p){let t=c[e],n=l[e*4+0],r=l[e*4+1],i=l[e*4+2],a=l[e*4+3],u=0;n>=0&&(u+=s[n]),r>=0&&(u+=s[r]),i>=0&&(u+=s[i]),a>=0&&(u+=s[a]),s[t]=f*((o+u)/4)+(1-f)*s[t]}for(let e of m){let t=c[e],n=l[e*4+0],r=l[e*4+1],i=l[e*4+2],a=l[e*4+3],u=0;n>=0&&(u+=s[n]),r>=0&&(u+=s[r]),i>=0&&(u+=s[i]),a>=0&&(u+=s[a]),s[t]=f*((o+u)/4)+(1-f)*s[t]}}let h=new Float32Array(r*i);for(let e=0;e<3;e++){h.set(s);for(let e=0;e<u;e++){let t=c[e],n=l[e*4+0],r=l[e*4+1],i=l[e*4+2],a=l[e*4+3],o=0,u=0;n>=0&&(o+=h[n],u++),r>=0&&(o+=h[r],u++),i>=0&&(o+=h[i],u++),a>=0&&(o+=h[a],u++),s[t]=u>0?(h[t]+o/u)*.5:h[t]}}if(ct.measurePerformance){let e=performance.now()-d;console.log(`[Optimized Poisson Solver (SOR \u03C9=${f})]`),console.log(`  Working size: ${r}\xD7${i}`),console.log(`  Iterations: ${a}`),console.log(`  Time: ${e.toFixed(2)}ms`),console.log(`  Interior pixels processed: ${u}`),console.log(`  Speed: ${(a*u/(e*1e3)).toFixed(2)} Mpixels/sec`)}return s}var ft={none:0,circle:1,daisy:2,diamond:3,metaballs:4};function O(e){if(Array.isArray(e))return e.length===4?e:e.length===3?[...e,1]:vt;if(typeof e!=`string`)return vt;let t,n,r,i=1;if(e.startsWith(`#`))[t,n,r,i]=pt(e);else if(e.startsWith(`rgb`)){let a=mt(e);if(a===null)return vt;[t,n,r,i]=a}else if(e.startsWith(`hsl`)){let a=ht(e);if(a===null)return vt;[t,n,r,i]=gt(a)}else return console.error(`Unsupported color format`,e),vt;return[_t(t,0,1),_t(n,0,1),_t(r,0,1),_t(i,0,1)]}function pt(e){return e=e.replace(/^#/,``),(e.length===3||e.length===4)&&(e=e.split(``).map(e=>e+e).join(``)),e.length===6&&(e+=`ff`),/^[0-9a-f]{8}$/i.test(e)?[parseInt(e.slice(0,2),16)/255,parseInt(e.slice(2,4),16)/255,parseInt(e.slice(4,6),16)/255,parseInt(e.slice(6,8),16)/255]:(console.warn(`Invalid hex color`),vt)}function mt(e){let t=e.match(/^rgba?\s*\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*(?:,\s*([0-9.]+))?\s*\)$/i);return t?[parseInt(t[1]??`0`)/255,parseInt(t[2]??`0`)/255,parseInt(t[3]??`0`)/255,t[4]===void 0?1:parseFloat(t[4])]:null}function ht(e){let t=e.match(/^hsla?\s*\(\s*(\d+)\s*,\s*(\d+)%\s*,\s*(\d+)%\s*(?:,\s*([0-9.]+))?\s*\)$/i);return t?[parseInt(t[1]??`0`),parseInt(t[2]??`0`),parseInt(t[3]??`0`),t[4]===void 0?1:parseFloat(t[4])]:null}function gt(e){let[t,n,r,i]=e,a=t/360,o=n/100,s=r/100,c,l,u;if(n===0)c=l=u=s;else{let e=(e,t,n)=>(n<0&&(n+=1),n>1&&--n,n<1/6?e+(t-e)*6*n:n<1/2?t:n<2/3?e+(t-e)*(2/3-n)*6:e),t=s<.5?s*(1+o):s+o-s*o,n=2*s-t;c=e(n,t,a+1/3),l=e(n,t,a),u=e(n,t,a-1/3)}return[c,l,u,i]}var _t=(e,t,n)=>Math.min(Math.max(e,t),n),vt=[.5,.5,.5,1];function yt(){if(typeof window>`u`)return;let e=new Image;return e.src=bt,e}var bt=`data:image/png;base64,`,k=t(e(),1);function xt(e){let t=k.useRef(void 0),n=k.useCallback(t=>{let n=e.map(e=>{if(e!=null){if(typeof e==`function`){let n=e,r=n(t);return typeof r==`function`?r:()=>{n(null)}}return e.current=t,()=>{e.current=null}}});return()=>{n.forEach(e=>e?.())}},e);return k.useMemo(()=>e.every(e=>e==null)?null:e=>{t.current&&=(t.current(),void 0),e!=null&&(t.current=n(e))},e)}function St(e){if(e.naturalWidth<1024&&e.naturalHeight<1024){if(e.naturalWidth<1||e.naturalHeight<1)return;let t=e.naturalWidth/e.naturalHeight;e.width=Math.round(t>1?1024*t:1024),e.height=Math.round(t>1?1024:1024/t)}}var A=n();async function Ct(e){let t={},n=[],r=e=>{try{return e.startsWith(`/`)||new URL(e),!0}catch{return!1}},i=e=>{try{return!e.startsWith(`/`)&&new URL(e,window.location.origin).origin!==window.location.origin}catch{return!1}};return Object.entries(e).forEach(([e,a])=>{if(typeof a==`string`){let o=a||`data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==`;if(!r(o)){console.warn(`Uniform "${e}" has invalid URL "${o}". Skipping image loading.`);return}let s=new Promise((n,r)=>{let a=new Image;i(o)&&(a.crossOrigin=`anonymous`),a.onload=()=>{St(a),t[e]=a,n()},a.onerror=()=>{console.error(`Could not set uniforms. Failed to load image at ${o}`),r()},a.src=o});n.push(s)}else if(a instanceof HTMLImageElement){let r=a.decode().then(()=>{St(a),t[e]=a});n.push(r)}else t[e]=a}),await Promise.all(n),t}var j=(0,k.forwardRef)(function({fragmentShader:e,uniforms:t,webGlContextAttributes:n,speed:r=0,frame:i=0,width:o,height:s,minPixelRatio:c,maxPixelCount:l,mipmaps:u,style:d,...f},p){let[m,h]=(0,k.useState)(!1),g=(0,k.useRef)(null),_=(0,k.useRef)(null),v=(0,k.useRef)(n);return(0,k.useEffect)(()=>((async()=>{let n=await Ct(t);g.current&&!_.current&&(_.current=new a(g.current,e,n,v.current,r,i,c,l,u),h(!0))})(),()=>{_.current?.dispose(),_.current=null}),[e]),(0,k.useEffect)(()=>{let e=!1;return(async()=>{let n=await Ct(t);e||_.current?.setUniforms(n)})(),()=>{e=!0}},[t,m]),(0,k.useEffect)(()=>{_.current?.setSpeed(r)},[r,m]),(0,k.useEffect)(()=>{_.current?.setMaxPixelCount(l)},[l,m]),(0,k.useEffect)(()=>{_.current?.setMinPixelRatio(c)},[c,m]),(0,k.useEffect)(()=>{_.current?.setFrame(i)},[i,m]),(0,A.jsx)(`div`,{ref:xt([g,p]),style:o!==void 0||s!==void 0?{width:typeof o==`string`&&isNaN(+o)===!1?+o:o,height:typeof s==`string`&&isNaN(+s)===!1?+s:s,...d}:d,...f})});j.displayName=`ShaderMount`;function M(e,t){if(Object.keys(e).length!==Object.keys(t).length)return!1;for(let n in e){if(n===`colors`){let n=Array.isArray(e.colors),r=Array.isArray(t.colors);if(!n||!r){if(Object.is(e.colors,t.colors)===!1)return!1;continue}if(e.colors?.length!==t.colors?.length||!e.colors?.every((e,n)=>e===t.colors?.[n]))return!1;continue}if(Object.is(e[n],t[n])===!1)return!1}return!0}var wt={name:`Default`,params:{...f,speed:1,frame:0,colors:[`#e0eaff`,`#241d9a`,`#f75092`,`#9f50d3`],distortion:.8,swirl:.1,grainMixer:0,grainOverlay:0}},Tt={name:`Purple`,params:{...f,speed:.6,frame:0,colors:[`#aaa7d7`,`#3c2b8e`],distortion:1,swirl:1,grainMixer:0,grainOverlay:0}},Et={name:`Beach`,params:{...f,speed:.1,frame:0,colors:[`#bcecf6`,`#00aaff`,`#00f7ff`,`#ffd447`],distortion:.8,swirl:.35,grainMixer:0,grainOverlay:0}},Dt=[wt,{name:`Ink`,params:{...f,speed:1,frame:0,colors:[`#ffffff`,`#000000`],distortion:1,swirl:.2,rotation:90,grainMixer:0,grainOverlay:0}},Tt,Et],Ot=(0,k.memo)(function({speed:e=wt.params.speed,frame:t=wt.params.frame,colors:n=wt.params.colors,distortion:r=wt.params.distortion,swirl:i=wt.params.swirl,grainMixer:a=wt.params.grainMixer,grainOverlay:o=wt.params.grainOverlay,fit:s=wt.params.fit,rotation:c=wt.params.rotation,scale:l=wt.params.scale,originX:u=wt.params.originX,originY:d=wt.params.originY,offsetX:f=wt.params.offsetX,offsetY:p=wt.params.offsetY,worldWidth:h=wt.params.worldWidth,worldHeight:g=wt.params.worldHeight,..._}){let v={u_colors:n.map(O),u_colorsCount:n.length,u_distortion:r,u_swirl:i,u_grainMixer:a,u_grainOverlay:o,u_fit:m[s],u_rotation:c,u_scale:l,u_offsetX:f,u_offsetY:p,u_originX:u,u_originY:d,u_worldWidth:h,u_worldHeight:g};return(0,A.jsx)(j,{..._,speed:e,frame:t,fragmentShader:T,uniforms:v})},M),N={name:`Default`,params:{...f,speed:.5,frame:0,colorBack:`#000000`,colors:[`#ffffff`],noiseScale:3,noiseIterations:8,radius:.25,thickness:.65,innerShape:.7,scale:.8}},kt={name:`Solar`,params:{...f,speed:1,frame:0,colorBack:`#000000`,colors:[`#ffffff`,`#ffca0a`,`#fc6203`,`#fc620366`],noiseScale:2,noiseIterations:3,radius:.4,thickness:.8,innerShape:4,scale:2,offsetY:1}},At=[N,{name:`Line`,params:{...f,frame:0,colorBack:`#000000`,colors:[`#4540a4`,`#1fe8ff`],noiseScale:1.1,noiseIterations:2,radius:.38,thickness:.01,innerShape:.88,speed:4}},kt,{name:`Cloud`,params:{...f,frame:0,colorBack:`#81ADEC`,colors:[`#ffffff`],noiseScale:3,noiseIterations:10,radius:.5,thickness:.65,innerShape:.85,speed:.5,scale:2.5}}],jt=(0,k.memo)(function({speed:e=N.params.speed,frame:t=N.params.frame,colorBack:n=N.params.colorBack,colors:r=N.params.colors,noiseScale:i=N.params.noiseScale,thickness:a=N.params.thickness,radius:o=N.params.radius,innerShape:s=N.params.innerShape,noiseIterations:c=N.params.noiseIterations,fit:l=N.params.fit,scale:u=N.params.scale,rotation:d=N.params.rotation,originX:f=N.params.originX,originY:p=N.params.originY,offsetX:h=N.params.offsetX,offsetY:g=N.params.offsetY,worldWidth:_=N.params.worldWidth,worldHeight:v=N.params.worldHeight,...y}){let b={u_colorBack:O(n),u_colors:r.map(O),u_colorsCount:r.length,u_noiseScale:i,u_thickness:a,u_radius:o,u_innerShape:s,u_noiseIterations:c,u_noiseTexture:yt(),u_fit:m[l],u_scale:u,u_rotation:d,u_offsetX:h,u_offsetY:g,u_originX:f,u_originY:p,u_worldWidth:_,u_worldHeight:v};return(0,A.jsx)(j,{...y,speed:e,frame:t,fragmentShader:ee,uniforms:b})},M),Mt={name:`Default`,params:{...p,speed:1,frame:0,colorFront:`#ffffff`,colorMid:`#47a6ff`,colorBack:`#000000`,brightness:.05,contrast:.3}},Nt=[Mt,{name:`Sensation`,params:{...p,speed:1,frame:0,colorFront:`#00c8ff`,colorMid:`#fbff00`,colorBack:`#8b42ff`,brightness:.19,contrast:.12,scale:3}},{name:`Bloodstream`,params:{...p,speed:1,frame:0,colorFront:`#ff0000`,colorMid:`#ff0000`,colorBack:`#ffffff`,brightness:.24,contrast:.17,scale:.7}},{name:`Ghost`,params:{...p,speed:1,frame:0,colorFront:`#ffffff`,colorMid:`#000000`,colorBack:`#ffffff`,brightness:0,contrast:1,scale:.55}}],Pt=(0,k.memo)(function({speed:e=Mt.params.speed,frame:t=Mt.params.frame,colorFront:n=Mt.params.colorFront,colorMid:r=Mt.params.colorMid,colorBack:i=Mt.params.colorBack,brightness:a=Mt.params.brightness,contrast:o=Mt.params.contrast,fit:s=Mt.params.fit,scale:c=Mt.params.scale,rotation:l=Mt.params.rotation,originX:u=Mt.params.originX,originY:d=Mt.params.originY,offsetX:f=Mt.params.offsetX,offsetY:p=Mt.params.offsetY,worldWidth:h=Mt.params.worldWidth,worldHeight:g=Mt.params.worldHeight,..._}){let v={u_colorFront:O(n),u_colorMid:O(r),u_colorBack:O(i),u_brightness:a,u_contrast:o,u_fit:m[s],u_scale:c,u_rotation:l,u_offsetX:f,u_offsetY:p,u_originX:u,u_originY:d,u_worldWidth:h,u_worldHeight:g};return(0,A.jsx)(j,{..._,speed:e,frame:t,fragmentShader:te,uniforms:v})},M),Ft={name:`Default`,params:{...p,speed:1.5,frame:0,colorBack:`#000000`,colors:[`#ffc96b`,`#ff6200`,`#ff2f00`,`#421100`,`#1a0000`],size:1,sizeRange:0,spreading:1,stepsPerColor:4}},It={name:`Shine`,params:{...p,speed:.1,frame:0,colors:[`#ffffff`,`#006aff`,`#fff675`],colorBack:`#000000`,stepsPerColor:4,size:.3,sizeRange:.2,spreading:1,scale:.4}},Lt=[Ft,{name:`Bubbles`,params:{...p,speed:.4,frame:0,colors:[`#D0D2D5`],colorBack:`#989CA4`,stepsPerColor:2,size:.9,sizeRange:.7,spreading:1,scale:1.64}},It,{name:`Hallucinatory`,params:{...p,speed:5,frame:0,colors:[`#000000`],colorBack:`#ffe500`,stepsPerColor:2,size:.65,sizeRange:0,spreading:.3,scale:.5}}],Rt=(0,k.memo)(function({speed:e=Ft.params.speed,frame:t=Ft.params.frame,colorBack:n=Ft.params.colorBack,colors:r=Ft.params.colors,size:i=Ft.params.size,sizeRange:a=Ft.params.sizeRange,spreading:o=Ft.params.spreading,stepsPerColor:s=Ft.params.stepsPerColor,fit:c=Ft.params.fit,scale:l=Ft.params.scale,rotation:u=Ft.params.rotation,originX:d=Ft.params.originX,originY:f=Ft.params.originY,offsetX:p=Ft.params.offsetX,offsetY:h=Ft.params.offsetY,worldWidth:g=Ft.params.worldWidth,worldHeight:_=Ft.params.worldHeight,...v}){let y={u_colorBack:O(n),u_colors:r.map(O),u_colorsCount:r.length,u_size:i,u_sizeRange:a,u_spreading:o,u_stepsPerColor:s,u_noiseTexture:yt(),u_fit:m[c],u_scale:l,u_rotation:u,u_offsetX:p,u_offsetY:h,u_originX:d,u_originY:f,u_worldWidth:g,u_worldHeight:_};return(0,A.jsx)(j,{...v,speed:e,frame:t,fragmentShader:re,uniforms:y})},M),P={name:`Default`,params:{...p,colorBack:`#000000`,colorFill:`#ffffff`,colorStroke:`#ffaa00`,size:2,gapX:32,gapY:32,strokeWidth:0,sizeRange:0,opacityRange:0,shape:`circle`}},zt=[P,{name:`Triangles`,params:{...p,colorBack:`#ffffff`,colorFill:`#ffffff`,colorStroke:`#808080`,size:5,gapX:32,gapY:32,strokeWidth:1,sizeRange:0,opacityRange:0,shape:`triangle`}},{name:`Tree line`,params:{...p,colorBack:`#f4fce7`,colorFill:`#052e19`,colorStroke:`#000000`,size:8,gapX:20,gapY:90,strokeWidth:0,sizeRange:1,opacityRange:.6,shape:`circle`}},{name:`Wallpaper`,params:{...p,colorBack:`#204030`,colorFill:`#000000`,colorStroke:`#bd955b`,size:9,gapX:32,gapY:32,strokeWidth:1,sizeRange:0,opacityRange:0,shape:`diamond`}}],Bt=(0,k.memo)(function({colorBack:e=P.params.colorBack,colorFill:t=P.params.colorFill,colorStroke:n=P.params.colorStroke,size:r=P.params.size,gapX:i=P.params.gapX,gapY:a=P.params.gapY,strokeWidth:o=P.params.strokeWidth,sizeRange:s=P.params.sizeRange,opacityRange:c=P.params.opacityRange,shape:l=P.params.shape,fit:u=P.params.fit,scale:d=P.params.scale,rotation:f=P.params.rotation,originX:p=P.params.originX,originY:h=P.params.originY,offsetX:g=P.params.offsetX,offsetY:_=P.params.offsetY,worldWidth:v=P.params.worldWidth,worldHeight:y=P.params.worldHeight,maxPixelCount:b=6016*3384,...x}){let S={u_colorBack:O(e),u_colorFill:O(t),u_colorStroke:O(n),u_dotSize:r,u_gapX:i,u_gapY:a,u_strokeWidth:o,u_sizeRange:s,u_opacityRange:c,u_shape:ae[l],u_fit:m[u],u_scale:d,u_rotation:f,u_offsetX:g,u_offsetY:_,u_originX:p,u_originY:h,u_worldWidth:v,u_worldHeight:y};return(0,A.jsx)(j,{...x,maxPixelCount:b,fragmentShader:ie,uniforms:S})},M),Vt={name:`Default`,params:{...p,scale:.6,speed:.5,frame:0,colors:[`#4449CF`,`#FFD1E0`,`#F94446`,`#FFD36B`,`#FFFFFF`],stepsPerColor:2,softness:0}},Ht={name:`Bubblegum`,params:{...p,speed:2,frame:0,colors:[`#ffffff`,`#ff9e9e`,`#5f57ff`,`#00f7ff`],stepsPerColor:1,softness:1,scale:1.6}},Ut=[Vt,{name:`Spots`,params:{...p,speed:.6,frame:0,colors:[`#ff7b00`,`#f9ffeb`,`#320d82`],stepsPerColor:1,softness:0,scale:1}},{name:`First contact`,params:{...p,speed:2,frame:0,colors:[`#e8cce6`,`#120d22`,`#442c44`,`#e6baba`,`#fff5f5`],stepsPerColor:2,softness:0,scale:.2}},Ht],Wt=(0,k.memo)(function({speed:e=Vt.params.speed,frame:t=Vt.params.frame,colors:n=Vt.params.colors,stepsPerColor:r=Vt.params.stepsPerColor,softness:i=Vt.params.softness,fit:a=Vt.params.fit,scale:o=Vt.params.scale,rotation:s=Vt.params.rotation,originX:c=Vt.params.originX,originY:l=Vt.params.originY,offsetX:u=Vt.params.offsetX,offsetY:d=Vt.params.offsetY,worldWidth:f=Vt.params.worldWidth,worldHeight:p=Vt.params.worldHeight,...h}){let g={u_colors:n.map(O),u_colorsCount:n.length,u_stepsPerColor:r,u_softness:i,u_fit:m[a],u_scale:o,u_rotation:s,u_offsetX:u,u_offsetY:d,u_originX:c,u_originY:l,u_worldWidth:f,u_worldHeight:p};return(0,A.jsx)(j,{...h,speed:e,frame:t,fragmentShader:oe,uniforms:g})},M),Gt={name:`Default`,params:{...f,scale:1,speed:1,frame:0,colorBack:`#000000`,colors:[`#6e33cc`,`#ff5500`,`#ffc105`,`#ffc800`,`#f585ff`],count:10,size:.83}},Kt={name:`Ink Drops`,params:{...f,scale:1,speed:2,frame:0,colorBack:`#ffffff00`,colors:[`#000000`],count:18,size:.1}},qt={name:`Background`,params:{...f,speed:.5,frame:0,colors:[`#ae00ff`,`#00ff95`,`#ffc105`],colorBack:`#2a273f`,count:13,size:.81,scale:4,rotation:0,offsetX:-.3}},Jt=[Gt,Kt,{name:`Solar`,params:{...f,speed:1,frame:0,colors:[`#ffc800`,`#ff5500`,`#ffc105`],colorBack:`#102f84`,count:7,size:.75,scale:1}},qt],Yt=(0,k.memo)(function({speed:e=Gt.params.speed,frame:t=Gt.params.frame,colorBack:n=Gt.params.colorBack,colors:r=Gt.params.colors,size:i=Gt.params.size,count:a=Gt.params.count,fit:o=Gt.params.fit,rotation:s=Gt.params.rotation,scale:c=Gt.params.scale,originX:l=Gt.params.originX,originY:u=Gt.params.originY,offsetX:d=Gt.params.offsetX,offsetY:f=Gt.params.offsetY,worldWidth:p=Gt.params.worldWidth,worldHeight:h=Gt.params.worldHeight,...g}){let _={u_colorBack:O(n),u_colors:r.map(O),u_colorsCount:r.length,u_size:i,u_count:a,u_noiseTexture:yt(),u_fit:m[o],u_rotation:s,u_scale:c,u_offsetX:d,u_offsetY:f,u_originX:l,u_originY:u,u_worldWidth:p,u_worldHeight:h};return(0,A.jsx)(j,{...g,speed:e,frame:t,fragmentShader:ce,uniforms:_})},M),Xt={name:`Default`,params:{...p,scale:.6,colorFront:`#ffbb00`,colorBack:`#000000`,shape:0,frequency:.5,amplitude:.5,spacing:1.2,proportion:.1,softness:0}},Zt=[Xt,{name:`Groovy`,params:{...p,scale:5,rotation:90,colorFront:`#fcfcee`,colorBack:`#ff896b`,shape:3,frequency:.2,amplitude:.25,spacing:1.17,proportion:.57,softness:0}},{name:`Tangled up`,params:{...p,scale:.5,rotation:0,colorFront:`#133a41`,colorBack:`#c2d8b6`,shape:2.07,frequency:.44,amplitude:.57,spacing:1.05,proportion:.75,softness:0}},{name:`Ride the wave`,params:{...p,scale:1.7,rotation:0,colorFront:`#fdffe6`,colorBack:`#1f1f1f`,shape:2.25,frequency:.2,amplitude:1,spacing:1.25,proportion:1,softness:0}}],Qt=(0,k.memo)(function({colorFront:e=Xt.params.colorFront,colorBack:t=Xt.params.colorBack,shape:n=Xt.params.shape,frequency:r=Xt.params.frequency,amplitude:i=Xt.params.amplitude,spacing:a=Xt.params.spacing,proportion:o=Xt.params.proportion,softness:s=Xt.params.softness,fit:c=Xt.params.fit,scale:l=Xt.params.scale,rotation:u=Xt.params.rotation,offsetX:d=Xt.params.offsetX,offsetY:f=Xt.params.offsetY,originX:p=Xt.params.originX,originY:h=Xt.params.originY,worldWidth:g=Xt.params.worldWidth,worldHeight:_=Xt.params.worldHeight,maxPixelCount:v=6016*3384,...y}){let b={u_colorFront:O(e),u_colorBack:O(t),u_shape:n,u_frequency:r,u_amplitude:i,u_spacing:a,u_proportion:o,u_softness:s,u_fit:m[c],u_scale:l,u_rotation:u,u_offsetX:d,u_offsetY:f,u_originX:p,u_originY:h,u_worldWidth:g,u_worldHeight:_};return(0,A.jsx)(j,{...y,fragmentShader:fe,uniforms:b})},M),F={name:`Default`,params:{...p,speed:.5,frame:0,colorBack:`#632ad5`,colorFront:`#fccff7`,proportion:.35,softness:.1,octaveCount:1,persistence:1,lacunarity:1.5}},$t=[F,{name:`Nintendo Water`,params:{...p,scale:1/.2,speed:.4,frame:0,colorBack:`#2d69d4`,colorFront:`#d1eefc`,proportion:.42,softness:0,octaveCount:2,persistence:.55,lacunarity:1.8}},{name:`Moss`,params:{...p,scale:1/.15,speed:.02,frame:0,colorBack:`#05ff4a`,colorFront:`#262626`,proportion:.65,softness:.35,octaveCount:6,persistence:1,lacunarity:2.55}},{name:`Worms`,params:{...p,scale:.9,speed:0,frame:0,colorBack:`#ffffff00`,colorFront:`#595959`,proportion:.5,softness:0,octaveCount:1,persistence:1,lacunarity:1.5}}],en=(0,k.memo)(function({speed:e=F.params.speed,frame:t=F.params.frame,colorFront:n=F.params.colorFront,colorBack:r=F.params.colorBack,proportion:i=F.params.proportion,softness:a=F.params.softness,octaveCount:o=F.params.octaveCount,persistence:s=F.params.persistence,lacunarity:c,fit:l=F.params.fit,worldWidth:u=F.params.worldWidth,worldHeight:d=F.params.worldHeight,scale:f=F.params.scale,rotation:p=F.params.rotation,originX:h=F.params.originX,originY:g=F.params.originY,offsetX:_=F.params.offsetX,offsetY:v=F.params.offsetY,...y}){let b={u_colorBack:O(r),u_colorFront:O(n),u_proportion:i,u_softness:a??F.params.softness,u_octaveCount:o??F.params.octaveCount,u_persistence:s??F.params.persistence,u_lacunarity:c??F.params.lacunarity,u_fit:m[l],u_scale:f,u_rotation:p,u_offsetX:_,u_offsetY:v,u_originX:h,u_originY:g,u_worldWidth:u,u_worldHeight:d};return(0,A.jsx)(j,{...y,speed:e,frame:t,fragmentShader:le,uniforms:b})},M),I={name:`Default`,params:{...p,speed:.5,frame:0,colors:[`#ff8247`,`#ffe53d`],stepsPerColor:3,colorGlow:`#ffffff`,colorGap:`#2e0000`,distortion:.4,gap:.04,glow:0,scale:.5}},tn={name:`Cells`,params:{...p,scale:.5,speed:.5,frame:0,colors:[`#ffffff`],stepsPerColor:1,colorGlow:`#ffffff`,colorGap:`#000000`,distortion:.5,gap:.03,glow:.8}},nn={name:`Bubbles`,params:{...p,scale:.75,speed:.5,frame:0,colors:[`#83c9fb`],stepsPerColor:1,colorGlow:`#ffffff`,colorGap:`#ffffff`,distortion:.4,gap:0,glow:1}},rn=[I,{name:`Lights`,params:{...p,scale:3.3,speed:.5,frame:0,colors:[`#fffffffc`,`#bbff00`,`#00ffff`],colorGlow:`#ff00d0`,colorGap:`#ff00d0`,stepsPerColor:2,distortion:.38,gap:0,glow:1}},tn,nn],an=(0,k.memo)(function({speed:e=I.params.speed,frame:t=I.params.frame,colors:n=I.params.colors,stepsPerColor:r=I.params.stepsPerColor,colorGlow:i=I.params.colorGlow,colorGap:a=I.params.colorGap,distortion:o=I.params.distortion,gap:s=I.params.gap,glow:c=I.params.glow,fit:l=I.params.fit,scale:u=I.params.scale,rotation:d=I.params.rotation,originX:f=I.params.originX,originY:p=I.params.originY,offsetX:h=I.params.offsetX,offsetY:g=I.params.offsetY,worldWidth:_=I.params.worldWidth,worldHeight:v=I.params.worldHeight,...y}){let b={u_colors:n.map(O),u_colorsCount:n.length,u_stepsPerColor:r,u_colorGlow:O(i),u_colorGap:O(a),u_distortion:o,u_gap:s,u_glow:c,u_noiseTexture:yt(),u_fit:m[l],u_scale:u,u_rotation:d,u_offsetX:h,u_offsetY:g,u_originX:f,u_originY:p,u_worldWidth:_,u_worldHeight:v};return(0,A.jsx)(j,{...y,speed:e,frame:t,fragmentShader:de,uniforms:b})},M),L={name:`Default`,params:{...p,rotation:0,speed:1,frame:0,colors:[`#121212`,`#9470ff`,`#121212`,`#8838ff`],proportion:.45,softness:1,distortion:.25,swirl:.8,swirlIterations:10,shapeScale:.1,shape:`checks`}},on=[L,{name:`Cauldron Pot`,params:{...p,scale:.9,rotation:160,speed:10,frame:0,colors:[`#a7e58b`,`#324472`,`#0a180d`],proportion:.64,softness:1.5,distortion:.2,swirl:.86,swirlIterations:7,shapeScale:.6,shape:`edge`}},{name:`Live Ink`,params:{...p,scale:1.2,rotation:44,offsetY:-.3,speed:2.5,frame:0,colors:[`#111314`,`#9faeab`,`#f3fee7`,`#f3fee7`],proportion:.05,softness:0,distortion:.25,swirl:.8,swirlIterations:10,shapeScale:.28,shape:`checks`}},{name:`Kelp`,params:{...p,scale:.8,rotation:50,speed:20,frame:0,colors:[`#dbff8f`,`#404f3e`,`#091316`],proportion:.67,softness:0,distortion:0,swirl:.2,swirlIterations:3,shapeScale:1,shape:`stripes`}},{name:`Nectar`,params:{...p,scale:2,offsetY:.6,rotation:0,speed:4.2,frame:0,colors:[`#151310`,`#d3a86b`,`#f0edea`],proportion:.24,softness:1,distortion:.21,swirl:.57,swirlIterations:10,shapeScale:.75,shape:`edge`}},{name:`Passion`,params:{...p,scale:2.5,rotation:1.35,speed:3,frame:0,colors:[`#3b1515`,`#954751`,`#ffc085`],proportion:.5,softness:1,distortion:.09,swirl:.9,swirlIterations:6,shapeScale:.25,shape:`checks`}}],sn=(0,k.memo)(function({speed:e=L.params.speed,frame:t=L.params.frame,colors:n=L.params.colors,proportion:r=L.params.proportion,softness:i=L.params.softness,distortion:a=L.params.distortion,swirl:o=L.params.swirl,swirlIterations:s=L.params.swirlIterations,shapeScale:c=L.params.shapeScale,shape:l=L.params.shape,fit:u=L.params.fit,scale:d=L.params.scale,rotation:f=L.params.rotation,originX:p=L.params.originX,originY:h=L.params.originY,offsetX:g=L.params.offsetX,offsetY:_=L.params.offsetY,worldWidth:v=L.params.worldWidth,worldHeight:y=L.params.worldHeight,...b}){let x={u_colors:n.map(O),u_colorsCount:n.length,u_proportion:r,u_softness:i,u_distortion:a,u_swirl:o,u_swirlIterations:s,u_shapeScale:c,u_shape:he[l],u_noiseTexture:yt(),u_scale:d,u_rotation:f,u_fit:m[u],u_offsetX:g,u_offsetY:_,u_originX:p,u_originY:h,u_worldWidth:v,u_worldHeight:y};return(0,A.jsx)(j,{...b,speed:e,frame:t,fragmentShader:me,uniforms:x})},M),R={name:`Default`,params:{...f,offsetX:0,offsetY:-.55,colorBack:`#000000`,colorBloom:`#0000ff`,colors:[`#a600ff6e`,`#6200fff0`,`#ffffff`,`#33fff5`],density:.3,spotty:.3,midIntensity:.4,midSize:.2,intensity:.8,bloom:.4,speed:.75,frame:0}},cn=[R,{name:`Warp`,params:{...f,colorBack:`#000000`,colorBloom:`#222288`,colors:[`#ff47d4`,`#ff8c00`,`#ffffff`],density:.45,spotty:.15,midIntensity:.4,midSize:.33,intensity:.79,bloom:.4,speed:2,frame:0}},{name:`Linear`,params:{...f,offsetX:.2,offsetY:-.8,colorBack:`#000000`,colorBloom:`#eeeeee`,colors:[`#ffffff1f`,`#ffffff3d`,`#ffffff29`],density:.41,spotty:.25,midSize:.1,midIntensity:.75,intensity:.79,bloom:1,speed:.5,frame:0}},{name:`Ether`,params:{...f,offsetX:-.6,colorBack:`#090f1d`,colorBloom:`#ffffff`,colors:[`#148effa6`,`#c4dffebe`,`#232a47`],density:.03,spotty:.77,midSize:.1,midIntensity:.6,intensity:.6,bloom:.6,speed:1,frame:0}}],ln=(0,k.memo)(function({speed:e=R.params.speed,frame:t=R.params.frame,colorBloom:n=R.params.colorBloom,colorBack:r=R.params.colorBack,colors:i=R.params.colors,density:a=R.params.density,spotty:o=R.params.spotty,midIntensity:s=R.params.midIntensity,midSize:c=R.params.midSize,intensity:l=R.params.intensity,bloom:u=R.params.bloom,fit:d=R.params.fit,scale:f=R.params.scale,rotation:p=R.params.rotation,originX:h=R.params.originX,originY:g=R.params.originY,offsetX:_=R.params.offsetX,offsetY:v=R.params.offsetY,worldWidth:y=R.params.worldWidth,worldHeight:b=R.params.worldHeight,...x}){let S={u_colorBloom:O(n),u_colorBack:O(r),u_colors:i.map(O),u_colorsCount:i.length,u_density:a,u_spotty:o,u_midIntensity:s,u_midSize:c,u_intensity:l,u_bloom:u,u_noiseTexture:yt(),u_fit:m[d],u_scale:f,u_rotation:p,u_offsetX:_,u_offsetY:v,u_originX:h,u_originY:g,u_worldWidth:y,u_worldHeight:b};return(0,A.jsx)(j,{...x,speed:e,frame:t,fragmentShader:_e,uniforms:S})},M),z={name:`Default`,params:{...p,scale:1,colorBack:`#001429`,colorFront:`#79D1FF`,density:1,distortion:0,strokeWidth:.5,strokeTaper:0,strokeCap:0,noise:0,noiseFrequency:0,softness:0,speed:1,frame:0}},un={name:`Droplet`,params:{...p,colorBack:`#effafe`,colorFront:`#bf40a0`,density:.9,distortion:0,strokeWidth:.75,strokeTaper:.18,strokeCap:1,noise:.74,noiseFrequency:.33,softness:.02,speed:1,frame:0}},dn=[z,{name:`Jungle`,params:{...p,scale:1.3,density:.5,colorBack:`#a0ef2a`,colorFront:`#288b18`,distortion:0,strokeWidth:.5,strokeTaper:0,strokeCap:0,noise:1,noiseFrequency:.25,softness:0,speed:.75,frame:0}},un,{name:`Swirl`,params:{...p,scale:.45,colorBack:`#b3e6d9`,colorFront:`#1a2b4d`,density:.2,distortion:0,strokeWidth:.5,strokeTaper:0,strokeCap:0,noise:0,noiseFrequency:.3,softness:.5,speed:1,frame:0}}],fn=(0,k.memo)(function({speed:e=z.params.speed,frame:t=z.params.frame,colorBack:n=z.params.colorBack,colorFront:r=z.params.colorFront,density:i=z.params.density,distortion:a=z.params.distortion,strokeWidth:o=z.params.strokeWidth,strokeTaper:s=z.params.strokeTaper,strokeCap:c=z.params.strokeCap,noiseFrequency:l=z.params.noiseFrequency,noise:u=z.params.noise,softness:d=z.params.softness,fit:f=z.params.fit,rotation:p=z.params.rotation,scale:h=z.params.scale,originX:g=z.params.originX,originY:_=z.params.originY,offsetX:v=z.params.offsetX,offsetY:y=z.params.offsetY,worldWidth:b=z.params.worldWidth,worldHeight:x=z.params.worldHeight,...S}){let C={u_colorBack:O(n),u_colorFront:O(r),u_density:i,u_distortion:a,u_strokeWidth:o,u_strokeTaper:s,u_strokeCap:c,u_noiseFrequency:l,u_noise:u,u_softness:d,u_fit:m[f],u_scale:h,u_rotation:p,u_offsetX:v,u_offsetY:y,u_originX:g,u_originY:_,u_worldWidth:b,u_worldHeight:x};return(0,A.jsx)(j,{...S,speed:e,frame:t,fragmentShader:ve,uniforms:C})},M),B={name:`Default`,params:{...f,speed:.32,frame:0,colorBack:`#330000`,colors:[`#ffd1d1`,`#ff8a8a`,`#660000`],bandCount:4,twist:.1,center:.2,proportion:.5,softness:0,noiseFrequency:.4,noise:.2}},pn={name:`Opening`,params:{...f,offsetX:-.4,offsetY:1,speed:.5,frame:0,colorBack:`#ff8b61`,colors:[`#fefff0`,`#ffd8bd`,`#ff8b61`],bandCount:2,twist:.3,center:.2,proportion:.5,softness:0,noiseFrequency:0,noise:0,scale:1}},mn=[B,{name:`007`,params:{...f,speed:1,frame:0,colorBack:`#E9E7DA`,colors:[`#000000`],bandCount:5,twist:.3,center:0,proportion:0,softness:0,noiseFrequency:.5,noise:0}},pn,{name:`Candy`,params:{...f,speed:1,frame:0,colorBack:`#ffcd66`,colors:[`#6bbceb`,`#d7b3ff`,`#ff9fff`],bandCount:2,twist:.15,center:.2,proportion:.5,softness:1,noiseFrequency:.5,noise:0}}],hn=(0,k.memo)(function({speed:e=B.params.speed,frame:t=B.params.frame,colorBack:n=B.params.colorBack,colors:r=B.params.colors,bandCount:i=B.params.bandCount,twist:a=B.params.twist,center:o=B.params.center,proportion:s=B.params.proportion,softness:c=B.params.softness,noiseFrequency:l=B.params.noiseFrequency,noise:u=B.params.noise,fit:d=B.params.fit,rotation:f=B.params.rotation,scale:p=B.params.scale,originX:h=B.params.originX,originY:g=B.params.originY,offsetX:_=B.params.offsetX,offsetY:v=B.params.offsetY,worldWidth:y=B.params.worldWidth,worldHeight:b=B.params.worldHeight,...x}){let S={u_colorBack:O(n),u_colors:r.map(O),u_colorsCount:r.length,u_bandCount:i,u_twist:a,u_center:o,u_proportion:s,u_softness:c,u_noiseFrequency:l,u_noise:u,u_fit:m[d],u_scale:p,u_rotation:f,u_offsetX:_,u_offsetY:v,u_originX:h,u_originY:g,u_worldWidth:y,u_worldHeight:b};return(0,A.jsx)(j,{...x,speed:e,frame:t,fragmentShader:be,uniforms:S})},M),gn={name:`Default`,params:{...p,speed:1,frame:0,scale:.6,colorBack:`#000000`,colorFront:`#00b2ff`,shape:`sphere`,type:`4x4`,size:2}},_n={name:`Sine Wave`,params:{...p,speed:1,frame:0,colorBack:`#730d54`,colorFront:`#00becc`,shape:`wave`,type:`4x4`,size:11,scale:1.2}},vn={name:`Bugs`,params:{...p,speed:1,frame:0,colorBack:`#000000`,colorFront:`#008000`,shape:`dots`,type:`random`,size:9}},yn={name:`Ripple`,params:{...f,speed:1,frame:0,colorBack:`#603520`,colorFront:`#c67953`,shape:`ripple`,type:`2x2`,size:3}},bn={name:`Swirl`,params:{...f,speed:1,frame:0,colorBack:`#00000000`,colorFront:`#47a8e1`,shape:`swirl`,type:`8x8`,size:2}},xn=[gn,{name:`Warp`,params:{...f,speed:1,frame:0,colorBack:`#301c2a`,colorFront:`#56ae6c`,shape:`warp`,type:`4x4`,size:2.5}},_n,yn,vn,bn],Sn=(0,k.memo)(function({speed:e=gn.params.speed,frame:t=gn.params.frame,colorBack:n=gn.params.colorBack,colorFront:r=gn.params.colorFront,shape:i=gn.params.shape,type:a=gn.params.type,pxSize:o,size:s=o===void 0?gn.params.size:o,fit:c=gn.params.fit,scale:l=gn.params.scale,rotation:u=gn.params.rotation,originX:d=gn.params.originX,originY:f=gn.params.originY,offsetX:p=gn.params.offsetX,offsetY:h=gn.params.offsetY,worldWidth:g=gn.params.worldWidth,worldHeight:_=gn.params.worldHeight,...v}){let y={u_colorBack:O(n),u_colorFront:O(r),u_shape:Se[i],u_type:Ce[a],u_pxSize:s,u_fit:m[c],u_scale:l,u_rotation:u,u_offsetX:p,u_offsetY:h,u_originX:d,u_originY:f,u_worldWidth:g,u_worldHeight:_};return(0,A.jsx)(j,{...v,speed:e,frame:t,fragmentShader:xe,uniforms:y})}),Cn={name:`Default`,params:{...f,speed:1,frame:0,colorBack:`#000000`,colors:[`#7300ff`,`#eba8ff`,`#00bfff`,`#2a00ff`],softness:.5,intensity:.5,noise:.25,shape:`corners`}},wn=[Cn,{name:`Wave`,params:{...p,speed:1,frame:0,colorBack:`#000a0f`,colors:[`#c4730b`,`#bdad5f`,`#d8ccc7`],softness:.7,intensity:.15,noise:.5,shape:`wave`}},{name:`Dots`,params:{...p,scale:.6,speed:1,frame:0,colorBack:`#0a0000`,colors:[`#6f0000`,`#0080ff`,`#f2ebc9`,`#33cc33`],softness:1,intensity:1,noise:.7,shape:`dots`}},{name:`Truchet`,params:{...p,speed:1,frame:0,colorBack:`#0a0000`,colors:[`#6f2200`,`#eabb7c`,`#39b523`],softness:0,intensity:.2,noise:1,shape:`truchet`}},{name:`Ripple`,params:{...f,scale:.5,speed:1,frame:0,colorBack:`#140a00`,colors:[`#6f2d00`,`#88ddae`,`#2c0b1d`],softness:.5,intensity:.5,noise:.5,shape:`ripple`}},{name:`Blob`,params:{...f,scale:1.3,speed:1,frame:0,colorBack:`#0f0e18`,colors:[`#3e6172`,`#a49b74`,`#568c50`],softness:0,intensity:.15,noise:.5,shape:`blob`}}],Tn=(0,k.memo)(function({speed:e=Cn.params.speed,frame:t=Cn.params.frame,colorBack:n=Cn.params.colorBack,colors:r=Cn.params.colors,softness:i=Cn.params.softness,intensity:a=Cn.params.intensity,noise:o=Cn.params.noise,shape:s=Cn.params.shape,fit:c=Cn.params.fit,scale:l=Cn.params.scale,rotation:u=Cn.params.rotation,originX:d=Cn.params.originX,originY:f=Cn.params.originY,offsetX:p=Cn.params.offsetX,offsetY:h=Cn.params.offsetY,worldWidth:g=Cn.params.worldWidth,worldHeight:_=Cn.params.worldHeight,...v}){let y={u_colorBack:O(n),u_colors:r.map(O),u_colorsCount:r.length,u_softness:i,u_intensity:a,u_noise:o,u_shape:Ee[s],u_noiseTexture:yt(),u_fit:m[c],u_scale:l,u_rotation:u,u_offsetX:p,u_offsetY:h,u_originX:d,u_originY:f,u_worldWidth:g,u_worldHeight:_};return(0,A.jsx)(j,{...v,speed:e,frame:t,fragmentShader:Te,uniforms:y})}),V={name:`Default`,params:{...f,speed:1,frame:0,scale:.6,colorBack:`#000000`,colors:[`#0dc1fd`,`#d915ef`,`#ff3f2ecc`],roundness:.25,thickness:.1,margin:0,marginLeft:0,marginRight:0,marginTop:0,marginBottom:0,aspectRatio:`auto`,softness:.75,intensity:.2,bloom:.25,spots:5,spotSize:.5,pulse:.25,smoke:.3,smokeSize:.6}},En=[V,{name:`Circle`,params:{...f,aspectRatio:`square`,scale:.6,speed:1,frame:0,colorBack:`#000000`,colors:[`#0dc1fd`,`#d915ef`,`#ff3f2ecc`],roundness:1,margin:0,marginLeft:0,marginRight:0,marginTop:0,marginBottom:0,thickness:0,softness:.75,intensity:.2,bloom:.45,spots:3,spotSize:.4,pulse:.5,smoke:1,smokeSize:0}},{name:`Northern lights`,params:{...f,speed:.18,scale:1.1,frame:0,colors:[`#4c4794`,`#774a7d`,`#12694a`,`#0aff78`,`#4733cc`],colorBack:`#0c182c`,roundness:0,thickness:1,softness:1,margin:0,marginLeft:0,marginRight:0,marginTop:0,marginBottom:0,aspectRatio:`auto`,intensity:.1,bloom:.2,spots:4,spotSize:.25,pulse:0,smoke:.32,smokeSize:.5}},{name:`Solid line`,params:{...f,speed:1,frame:0,colors:[`#81ADEC`],colorBack:`#00000000`,roundness:0,thickness:.05,margin:0,marginLeft:0,marginRight:0,marginTop:0,marginBottom:0,aspectRatio:`auto`,softness:0,intensity:0,bloom:.15,spots:4,spotSize:1,pulse:0,smoke:0,smokeSize:0}}],Dn=(0,k.memo)(function({speed:e=V.params.speed,frame:t=V.params.frame,colors:n=V.params.colors,colorBack:r=V.params.colorBack,roundness:i=V.params.roundness,thickness:a=V.params.thickness,aspectRatio:o=V.params.aspectRatio,softness:s=V.params.softness,bloom:c=V.params.bloom,intensity:l=V.params.intensity,spots:u=V.params.spots,spotSize:d=V.params.spotSize,pulse:f=V.params.pulse,smoke:p=V.params.smoke,smokeSize:h=V.params.smokeSize,margin:g,marginLeft:_=g??V.params.marginLeft,marginRight:v=g??V.params.marginRight,marginTop:y=g??V.params.marginTop,marginBottom:b=g??V.params.marginBottom,fit:x=V.params.fit,rotation:S=V.params.rotation,scale:C=V.params.scale,originX:w=V.params.originX,originY:T=V.params.originY,offsetX:E=V.params.offsetX,offsetY:ee=V.params.offsetY,worldWidth:te=V.params.worldWidth,worldHeight:ne=V.params.worldHeight,...re}){let ie={u_colorBack:O(r),u_colors:n.map(O),u_colorsCount:n.length,u_roundness:i,u_thickness:a,u_marginLeft:_,u_marginRight:v,u_marginTop:y,u_marginBottom:b,u_aspectRatio:ke[o],u_softness:s,u_intensity:l,u_bloom:c,u_spots:u,u_spotSize:d,u_pulse:f,u_smoke:p,u_smokeSize:h,u_noiseTexture:yt(),u_fit:m[x],u_rotation:S,u_scale:C,u_offsetX:E,u_offsetY:ee,u_originX:w,u_originY:T,u_worldWidth:te,u_worldHeight:ne};return(0,A.jsx)(j,{...re,speed:e,frame:t,fragmentShader:Oe,uniforms:ie})},M),H={name:`Default`,params:{...f,speed:.5,frame:0,colors:[`#ff9d00`,`#fd4f30`,`#809bff`,`#6d2eff`,`#333aff`,`#f15cff`,`#ffd557`],colorBack:`#000000`,angle1:0,angle2:0,length:1.1,edges:!1,blur:0,fadeIn:1,fadeOut:.3,gradient:0,density:3,scale:.8}},On=[H,{name:`Glass`,params:{...f,rotation:112,speed:1,frame:0,colors:[`#00cfff`,`#ff2d55`,`#34c759`,`#af52de`],colorBack:`#ffffff00`,angle1:.3,angle2:.3,length:1,edges:!0,blur:.25,fadeIn:.85,fadeOut:.3,gradient:0,density:1.6}},{name:`Gradient`,params:{...f,speed:.5,frame:0,colors:[`#f2ff00`,`#00000000`,`#00000000`,`#5a0283`,`#005eff`],colorBack:`#8ffff2`,angle1:.4,angle2:.4,length:3,edges:!1,blur:.5,fadeIn:1,fadeOut:.39,gradient:.78,density:1.65,scale:1.72,rotation:270,offsetX:.18}},{name:`Opening`,params:{...f,speed:2,frame:0,colors:[`#00ffff`],colorBack:`#570044`,angle1:-1,angle2:-1,length:.52,edges:!1,blur:0,fadeIn:0,fadeOut:1,gradient:0,density:2.21,scale:2.32,rotation:360,offsetX:-.3,offsetY:.6}}],kn=(0,k.memo)(function({speed:e=H.params.speed,frame:t=H.params.frame,colors:n=H.params.colors,colorBack:r=H.params.colorBack,angle1:i=H.params.angle1,angle2:a=H.params.angle2,length:o=H.params.length,edges:s=H.params.edges,blur:c=H.params.blur,fadeIn:l=H.params.fadeIn,fadeOut:u=H.params.fadeOut,density:d=H.params.density,gradient:f=H.params.gradient,fit:p=H.params.fit,scale:h=H.params.scale,rotation:g=H.params.rotation,originX:_=H.params.originX,originY:v=H.params.originY,offsetX:y=H.params.offsetX,offsetY:b=H.params.offsetY,worldWidth:x=H.params.worldWidth,worldHeight:S=H.params.worldHeight,...C}){let w={u_colors:n.map(O),u_colorsCount:n.length,u_colorBack:O(r),u_angle1:i,u_angle2:a,u_length:o,u_edges:s,u_blur:c,u_fadeIn:l,u_fadeOut:u,u_density:d,u_gradient:f,u_fit:m[p],u_scale:h,u_rotation:g,u_offsetX:y,u_offsetY:b,u_originX:_,u_originY:v,u_worldWidth:x,u_worldHeight:S};return(0,A.jsx)(j,{...C,speed:e,frame:t,fragmentShader:je,uniforms:w})},M),U={name:`Default`,params:{...f,rotation:270,speed:0,frame:0,colors:[`#ffad0a`,`#6200ff`,`#e2a3ff`,`#ff99fd`],positions:2,waveX:1,waveXShift:.6,waveY:1,waveYShift:.21,mixing:.93,grainMixer:0,grainOverlay:0}},An={name:`Sea`,params:{...f,speed:0,frame:0,colors:[`#013b65`,`#03738c`,`#a3d3ff`,`#f2faef`],positions:0,waveX:.53,waveXShift:0,waveY:.95,waveYShift:.64,mixing:.5,grainMixer:0,grainOverlay:0}},jn=[U,{name:`1960s`,params:{...f,speed:0,frame:0,colors:[`#000000`,`#082400`,`#b1aa91`,`#8e8c15`],positions:42,waveX:.45,waveXShift:0,waveY:1,waveYShift:0,mixing:0,grainMixer:.37,grainOverlay:.78}},{name:`Sunset`,params:{...f,speed:0,frame:0,colors:[`#264653`,`#9c2b2b`,`#f4a261`,`#ffffff`],positions:0,waveX:.6,waveXShift:.7,waveY:.7,waveYShift:.7,mixing:.5,grainMixer:0,grainOverlay:0}},An],Mn=(0,k.memo)(function({speed:e=U.params.speed,frame:t=U.params.frame,colors:n=U.params.colors,positions:r=U.params.positions,waveX:i=U.params.waveX,waveXShift:a=U.params.waveXShift,waveY:o=U.params.waveY,waveYShift:s=U.params.waveYShift,mixing:c=U.params.mixing,grainMixer:l=U.params.grainMixer,grainOverlay:u=U.params.grainOverlay,fit:d=U.params.fit,rotation:f=U.params.rotation,scale:p=U.params.scale,originX:h=U.params.originX,originY:g=U.params.originY,offsetX:_=U.params.offsetX,offsetY:v=U.params.offsetY,worldWidth:y=U.params.worldWidth,worldHeight:b=U.params.worldHeight,...x}){let S={u_colors:n.map(O),u_colorsCount:n.length,u_positions:r,u_waveX:i,u_waveXShift:a,u_waveY:o,u_waveYShift:s,u_mixing:c,u_grainMixer:l,u_grainOverlay:u,u_fit:m[d],u_rotation:f,u_scale:p,u_offsetX:_,u_offsetY:v,u_originX:h,u_originY:g,u_worldWidth:y,u_worldHeight:b};return(0,A.jsx)(j,{...x,speed:e,frame:t,fragmentShader:Ne,uniforms:S})},M),W={name:`Default`,params:{...f,scale:1,speed:0,frame:0,colorBack:`#000000`,colors:[`#00bbff`,`#00ffe1`,`#ffffff`],radius:.8,focalDistance:.99,focalAngle:0,falloff:.24,mixing:.5,distortion:0,distortionShift:0,distortionFreq:12,grainMixer:0,grainOverlay:0}},Nn={name:`Cross Section`,params:{...f,scale:1,speed:0,frame:0,colorBack:`#3d348b`,colors:[`#7678ed`,`#f7b801`,`#f18701`,`#37a066`],radius:1,focalDistance:0,focalAngle:0,falloff:0,mixing:0,distortion:1,distortionShift:0,distortionFreq:12,grainMixer:0,grainOverlay:0}},Pn={name:`Radial`,params:{...f,scale:1,speed:0,frame:0,colorBack:`#264653`,colors:[`#9c2b2b`,`#f4a261`,`#ffffff`],radius:1,focalDistance:0,focalAngle:0,falloff:0,mixing:1,distortion:0,distortionShift:0,distortionFreq:12,grainMixer:0,grainOverlay:0}},Fn=[W,{name:`Lo-Fi`,params:{...f,speed:0,frame:0,colorBack:`#2e1f27`,colors:[`#d72638`,`#3f88c5`,`#f49d37`],radius:1,focalDistance:0,focalAngle:0,falloff:.9,mixing:.7,distortion:0,distortionShift:0,distortionFreq:12,grainMixer:1,grainOverlay:.5}},Nn,Pn],In=(0,k.memo)(function({speed:e=W.params.speed,frame:t=W.params.frame,colorBack:n=W.params.colorBack,colors:r=W.params.colors,radius:i=W.params.radius,focalDistance:a=W.params.focalDistance,focalAngle:o=W.params.focalAngle,falloff:s=W.params.falloff,grainMixer:c=W.params.grainMixer,mixing:l=W.params.mixing,distortion:u=W.params.distortion,distortionShift:d=W.params.distortionShift,distortionFreq:f=W.params.distortionFreq,grainOverlay:p=W.params.grainOverlay,fit:h=W.params.fit,rotation:g=W.params.rotation,scale:_=W.params.scale,originX:v=W.params.originX,originY:y=W.params.originY,offsetX:b=W.params.offsetX,offsetY:x=W.params.offsetY,worldWidth:S=W.params.worldWidth,worldHeight:C=W.params.worldHeight,...w}){let T={u_colorBack:O(n),u_colors:r.map(O),u_colorsCount:r.length,u_radius:i,u_focalDistance:a,u_focalAngle:o,u_falloff:s,u_mixing:l,u_distortion:u,u_distortionShift:d,u_distortionFreq:f,u_grainMixer:c,u_grainOverlay:p,u_fit:m[h],u_rotation:g,u_scale:_,u_offsetX:b,u_offsetY:x,u_originX:v,u_originY:y,u_worldWidth:S,u_worldHeight:C};return(0,A.jsx)(j,{...w,speed:e,frame:t,fragmentShader:Fe,uniforms:T})},M),G={name:`Default`,params:{...f,fit:`cover`,scale:.6,speed:0,frame:0,colorFront:`#9fadbc`,colorBack:`#ffffff`,contrast:.3,roughness:.4,fiber:.3,fiberSize:.2,crumples:.3,crumpleSize:.35,folds:.65,foldCount:5,fade:0,drops:.2,seed:5.8}},Ln={name:`Abstract`,params:{...f,fit:`cover`,speed:0,frame:0,scale:.6,colorFront:`#00eeff`,colorBack:`#ff0a81`,contrast:.85,roughness:0,fiber:.1,fiberSize:.2,crumples:0,crumpleSize:.3,folds:1,foldCount:3,fade:0,drops:.2,seed:2.2}},Rn=[G,{name:`Cardboard`,params:{...f,fit:`cover`,speed:0,frame:0,scale:.6,colorFront:`#c7b89e`,colorBack:`#999180`,contrast:.4,roughness:0,fiber:.35,fiberSize:.14,crumples:.7,crumpleSize:.1,folds:0,foldCount:1,fade:0,drops:.1,seed:1.6}},Ln,{name:`Details`,params:{...f,speed:0,frame:0,fit:`cover`,scale:3,colorFront:`#00000000`,colorBack:`#00000000`,contrast:0,roughness:1,fiber:.27,fiberSize:.22,crumples:1,crumpleSize:.5,folds:1,foldCount:15,fade:0,drops:0,seed:6}}],zn=(0,k.memo)(function({speed:e=G.params.speed,frame:t=G.params.frame,colorFront:n=G.params.colorFront,colorBack:r=G.params.colorBack,image:i=``,contrast:a=G.params.contrast,roughness:o=G.params.roughness,fiber:s=G.params.fiber,crumples:c=G.params.crumples,folds:l=G.params.folds,drops:u=G.params.drops,seed:d=G.params.seed,fiberScale:f,fiberSize:p=f===void 0?G.params.fiberSize:.2/f,crumplesScale:h,crumpleSize:g=h===void 0?G.params.crumpleSize:.2/h,blur:_,fade:v=_===void 0?G.params.fade:_,foldsNumber:y,foldCount:b=y===void 0?G.params.foldCount:y,fit:x=G.params.fit,scale:S=G.params.scale,rotation:C=G.params.rotation,originX:w=G.params.originX,originY:T=G.params.originY,offsetX:E=G.params.offsetX,offsetY:ee=G.params.offsetY,worldWidth:te=G.params.worldWidth,worldHeight:ne=G.params.worldHeight,...re}){let ie=typeof window<`u`&&{u_noiseTexture:yt()},ae={u_image:i,u_colorFront:O(n),u_colorBack:O(r),u_contrast:a,u_roughness:o,u_fiber:s,u_fiberSize:p,u_crumples:c,u_crumpleSize:g,u_foldCount:b,u_folds:l,u_fade:v,u_drops:u,u_seed:d,...ie,u_fit:m[x],u_scale:S,u_rotation:C,u_offsetX:E,u_offsetY:ee,u_originX:w,u_originY:T,u_worldWidth:te,u_worldHeight:ne};return(0,A.jsx)(j,{...re,speed:e,frame:t,fragmentShader:Ie,mipmaps:[`u_image`],uniforms:ae})},M),K={name:`Default`,params:{...f,fit:`cover`,speed:0,frame:0,colorBack:`#00000000`,colorShadow:`#000000`,colorHighlight:`#ffffff`,shadows:.25,size:.5,angle:0,distortionShape:`prism`,highlights:.1,shape:`lines`,distortion:.5,shift:0,blur:0,edges:.25,stretch:0,margin:0,marginLeft:0,marginRight:0,marginTop:0,marginBottom:0,grainMixer:0,grainOverlay:0}},Bn={name:`Waves`,params:{...f,fit:`cover`,scale:1.2,speed:0,frame:0,colorBack:`#00000000`,colorShadow:`#000000`,colorHighlight:`#ffffff`,shadows:0,size:.9,angle:0,distortionShape:`contour`,highlights:0,shape:`wave`,distortion:.5,shift:0,blur:.1,edges:.5,stretch:1,margin:0,marginLeft:0,marginRight:0,marginTop:0,marginBottom:0,grainMixer:0,grainOverlay:.05}},Vn=[K,{name:`Abstract`,params:{...f,fit:`cover`,scale:4,speed:0,frame:0,colorBack:`#00000000`,colorShadow:`#000000`,colorHighlight:`#ffffff`,shadows:0,size:.7,angle:30,distortionShape:`flat`,highlights:0,shape:`linesIrregular`,distortion:1,shift:0,blur:1,edges:.5,stretch:1,margin:0,marginLeft:0,marginRight:0,marginTop:0,marginBottom:0,grainMixer:.1,grainOverlay:.1}},Bn,{name:`Folds`,params:{...f,fit:`cover`,speed:0,frame:0,colorBack:`#00000000`,colorShadow:`#000000`,colorHighlight:`#ffffff`,shadows:.4,size:.4,angle:0,distortionShape:`cascade`,highlights:0,shape:`lines`,distortion:.75,shift:0,blur:.25,edges:.5,stretch:0,margin:.1,marginLeft:.1,marginRight:.1,marginTop:.1,marginBottom:.1,grainMixer:0,grainOverlay:0}}],Hn=(0,k.memo)(function({speed:e=K.params.speed,frame:t=K.params.frame,colorBack:n=K.params.colorBack,colorShadow:r=K.params.colorShadow,colorHighlight:i=K.params.colorHighlight,image:a=``,shadows:o=K.params.shadows,angle:s=K.params.angle,distortion:c=K.params.distortion,distortionShape:l=K.params.distortionShape,highlights:u=K.params.highlights,shape:d=K.params.shape,shift:f=K.params.shift,blur:p=K.params.blur,edges:h=K.params.edges,margin:g,marginLeft:_=g??K.params.marginLeft,marginRight:v=g??K.params.marginRight,marginTop:y=g??K.params.marginTop,marginBottom:b=g??K.params.marginBottom,grainMixer:x=K.params.grainMixer,grainOverlay:S=K.params.grainOverlay,stretch:C=K.params.stretch,count:w,size:T=w===void 0?K.params.size:(1/(w*1.6))**(1/6)/.7-.5,fit:E=K.params.fit,scale:ee=K.params.scale,rotation:te=K.params.rotation,originX:ne=K.params.originX,originY:re=K.params.originY,offsetX:ie=K.params.offsetX,offsetY:ae=K.params.offsetY,worldWidth:D=K.params.worldWidth,worldHeight:oe=K.params.worldHeight,...se}){let ce={u_image:a,u_colorBack:O(n),u_colorShadow:O(r),u_colorHighlight:O(i),u_shadows:o,u_size:T,u_angle:s,u_distortion:c,u_shift:f,u_blur:p,u_edges:h,u_stretch:C,u_distortionShape:Be[l],u_highlights:u,u_shape:ze[d],u_marginLeft:_,u_marginRight:v,u_marginTop:y,u_marginBottom:b,u_grainMixer:x,u_grainOverlay:S,u_fit:m[E],u_scale:ee,u_rotation:te,u_offsetX:ie,u_offsetY:ae,u_originX:ne,u_originY:re,u_worldWidth:D,u_worldHeight:oe};return(0,A.jsx)(j,{...se,speed:e,frame:t,fragmentShader:Re,mipmaps:[`u_image`],uniforms:ce})}),q={name:`Default`,params:{...f,scale:.8,speed:1,frame:0,colorBack:`#909090`,colorHighlight:`#ffffff`,highlights:.07,layering:.5,edges:.8,waves:.3,caustic:.1,size:1}},Un={name:`Abstract`,params:{...f,fit:`cover`,scale:3,speed:1,frame:0,colorBack:`#909090`,colorHighlight:`#ffffff`,highlights:0,layering:0,edges:1,waves:1,caustic:.4,size:.15}},Wn={name:`Streaming`,params:{...f,fit:`contain`,scale:.4,speed:2,frame:0,colorBack:`#909090`,colorHighlight:`#ffffff`,highlights:0,layering:0,edges:0,waves:.5,caustic:0,size:.5}},Gn=[q,{name:`Slow-mo`,params:{...f,fit:`cover`,scale:1,speed:.1,frame:0,colorBack:`#909090`,colorHighlight:`#ffffff`,highlights:.4,layering:0,edges:0,waves:0,caustic:.2,size:.7}},Un,Wn],Kn=(0,k.memo)(function({speed:e=q.params.speed,frame:t=q.params.frame,colorBack:n=q.params.colorBack,colorHighlight:r=q.params.colorHighlight,image:i=``,highlights:a=q.params.highlights,layering:o=q.params.layering,waves:s=q.params.waves,edges:c=q.params.edges,caustic:l=q.params.caustic,effectScale:u,size:d=u===void 0?q.params.size:10/9/u-1/9,fit:f=q.params.fit,scale:p=q.params.scale,rotation:h=q.params.rotation,originX:g=q.params.originX,originY:_=q.params.originY,offsetX:v=q.params.offsetX,offsetY:y=q.params.offsetY,worldWidth:b=q.params.worldWidth,worldHeight:x=q.params.worldHeight,...S}){let C={u_image:i,u_colorBack:O(n),u_colorHighlight:O(r),u_highlights:a,u_layering:o,u_waves:s,u_edges:c,u_caustic:l,u_size:d,u_fit:m[f],u_rotation:h,u_scale:p,u_offsetX:v,u_offsetY:y,u_originX:g,u_originY:_,u_worldWidth:b,u_worldHeight:x};return(0,A.jsx)(j,{...S,speed:e,frame:t,fragmentShader:Le,mipmaps:[`u_image`],uniforms:C})},M),J={name:`Default`,params:{...f,fit:`cover`,speed:0,frame:0,colorFront:`#94ffaf`,colorBack:`#000c38`,colorHighlight:`#eaff94`,type:`8x8`,size:2,colorSteps:2,originalColors:!1,inverted:!1}},qn={name:`Retro`,params:{...f,fit:`cover`,speed:0,frame:0,colorFront:`#eeeeee`,colorBack:`#5452ff`,colorHighlight:`#eeeeee`,type:`2x2`,size:3,colorSteps:1,originalColors:!0,inverted:!1}},Jn=[J,{name:`Noise`,params:{...f,fit:`cover`,speed:0,frame:0,colorFront:`#a2997c`,colorBack:`#000000`,colorHighlight:`#ededed`,type:`random`,size:1,colorSteps:1,originalColors:!1,inverted:!1}},qn,{name:`Natural`,params:{...f,fit:`cover`,speed:0,frame:0,colorFront:`#ffffff`,colorBack:`#000000`,colorHighlight:`#ffffff`,type:`8x8`,size:2,colorSteps:5,originalColors:!0,inverted:!1}}],Yn=(0,k.memo)(function({speed:e=J.params.speed,frame:t=J.params.frame,colorFront:n=J.params.colorFront,colorBack:r=J.params.colorBack,colorHighlight:i=J.params.colorHighlight,image:a=``,type:o=J.params.type,colorSteps:s=J.params.colorSteps,originalColors:c=J.params.originalColors,inverted:l=J.params.inverted,pxSize:u,size:d=u===void 0?J.params.size:u,fit:f=J.params.fit,scale:p=J.params.scale,rotation:h=J.params.rotation,originX:g=J.params.originX,originY:_=J.params.originY,offsetX:v=J.params.offsetX,offsetY:y=J.params.offsetY,worldWidth:b=J.params.worldWidth,worldHeight:x=J.params.worldHeight,...S}){let C={u_image:a,u_colorFront:O(n),u_colorBack:O(r),u_colorHighlight:O(i),u_type:Ce[o],u_pxSize:d,u_colorSteps:s,u_originalColors:c,u_inverted:l,u_fit:m[f],u_rotation:h,u_scale:p,u_offsetX:v,u_offsetY:y,u_originX:g,u_originY:_,u_worldWidth:b,u_worldHeight:x};return(0,A.jsx)(j,{...S,speed:e,frame:t,fragmentShader:Ve,uniforms:C})},M),Y={name:`Default`,params:{...f,scale:1,fit:`contain`,speed:0,frame:0,spread:.6,bias:1,angle:0,perspective:.1,count:35,dispersion:1,dispersionShift:0,dispersionColor:.6,focusCenter:.8,focusEdges:1,swirl:.35,noise:0,noiseFrequency:.25,noiseOffset:0,lensBulge:0,lensCircle:0,grainMixer:0,grainOverlay:0,imageX:0,imageY:0}},Xn=[Y,{name:`Fisheye`,params:{...f,scale:1,fit:`contain`,speed:0,frame:0,spread:.7,bias:1,angle:0,perspective:.75,count:15,dispersion:1,dispersionShift:0,dispersionColor:.7,focusCenter:.9,focusEdges:1,swirl:1,noise:0,noiseFrequency:.25,noiseOffset:0,lensBulge:.95,lensCircle:1,grainMixer:.5,grainOverlay:0,imageX:0,imageY:0}},{name:`Duotone`,params:{...f,fit:`contain`,speed:0,frame:0,spread:.1,bias:0,angle:0,perspective:0,count:2,dispersion:1,dispersionShift:0,dispersionColor:.1,focusCenter:0,focusEdges:0,swirl:0,noise:0,noiseFrequency:.25,noiseOffset:0,lensBulge:0,lensCircle:0,grainMixer:.1,grainOverlay:.3,imageX:0,imageY:0}},{name:`Smoke`,params:{...f,scale:1,fit:`contain`,speed:0,frame:0,spread:.65,bias:-1,angle:0,perspective:.4,count:50,dispersion:0,dispersionShift:0,dispersionColor:.31,focusCenter:0,focusEdges:.6,swirl:0,noise:1,noiseFrequency:.35,noiseOffset:.3,lensBulge:0,lensCircle:0,grainMixer:0,grainOverlay:0,imageX:0,imageY:0}}],Zn=(0,k.memo)(function({speed:e=Y.params.speed,frame:t=Y.params.frame,image:n=``,spread:r=Y.params.spread,bias:i=Y.params.bias,angle:a=Y.params.angle,perspective:o=Y.params.perspective,count:s=Y.params.count,dispersion:c=Y.params.dispersion,dispersionShift:l=Y.params.dispersionShift,dispersionColor:u=Y.params.dispersionColor,focusCenter:d=Y.params.focusCenter,focusEdges:f=Y.params.focusEdges,swirl:p=Y.params.swirl,noise:h=Y.params.noise,noiseFrequency:g=Y.params.noiseFrequency,noiseOffset:_=Y.params.noiseOffset,lensBulge:v=Y.params.lensBulge,lensCircle:y=Y.params.lensCircle,grainMixer:b=Y.params.grainMixer,grainOverlay:x=Y.params.grainOverlay,imageX:S=Y.params.imageX,imageY:C=Y.params.imageY,fit:w=Y.params.fit,scale:T=Y.params.scale,rotation:E=Y.params.rotation,originX:ee=Y.params.originX,originY:te=Y.params.originY,offsetX:ne=Y.params.offsetX,offsetY:re=Y.params.offsetY,worldWidth:ie=Y.params.worldWidth,worldHeight:ae=Y.params.worldHeight,...D}){let oe={u_image:n,u_spread:r,u_bias:i,u_angle:a,u_perspective:o,u_count:s,u_dispersion:c,u_dispersionShift:l,u_dispersionColor:u,u_focusCenter:d,u_focusEdges:f,u_swirl:p,u_noise:h,u_noiseFrequency:g,u_noiseOffset:_,u_lensBulge:v,u_lensCircle:y,u_grainMixer:b,u_grainOverlay:x,u_imageX:S,u_imageY:C,u_fit:m[w],u_rotation:E,u_scale:T,u_offsetX:ne,u_offsetY:re,u_originX:ee,u_originY:te,u_worldWidth:ie,u_worldHeight:ae};return(0,A.jsx)(j,{...D,speed:e,frame:t,fragmentShader:Ue,mipmaps:[`u_image`],uniforms:oe})},M),Qn=`data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==`,$n=e=>typeof e==`object`&&typeof e.then==`function`,er=[];function tr(e,t){if(e===t)return!0;if(!e||!t)return!1;let n=e.length;if(t.length!==n)return!1;for(let r=0;r<n;r++)if(e[r]!==t[r])return!1;return!0}function nr(e,t=null){t===null&&(t=[e]);for(let e of er)if(tr(t,e.keys)){if(Object.prototype.hasOwnProperty.call(e,`error`))throw e.error;if(Object.prototype.hasOwnProperty.call(e,`response`))return e.response;throw e.promise}let n={keys:t,promise:($n(e)?e:e(...t)).then(e=>{n.response=e}).catch(e=>n.error=e)};throw er.push(n),n.promise}var rr=(e,t)=>nr(e,t),ir={name:`Default`,params:{...f,scale:.75,speed:1,frame:0,contour:.5,angle:0,noise:0,innerGlow:.5,outerGlow:.5,colorBack:`#000000`,colors:[`#11206a`,`#1f3ba2`,`#2f63e7`,`#6bd7ff`,`#ffe679`,`#ff991e`,`#ff4c00`]}},ar=[ir,{name:`Sepia`,params:{...f,scale:.75,speed:.5,frame:0,contour:.5,angle:0,noise:.75,innerGlow:.5,outerGlow:.5,colorBack:`#000000`,colors:[`#997F45`,`#ffffff`]}}],or=(0,k.memo)(function({speed:e=ir.params.speed,frame:t=ir.params.frame,image:n=``,contour:r=ir.params.contour,angle:i=ir.params.angle,noise:a=ir.params.noise,innerGlow:o=ir.params.innerGlow,outerGlow:s=ir.params.outerGlow,colorBack:c=ir.params.colorBack,colors:l=ir.params.colors,suspendWhenProcessingImage:u=!1,fit:d=ir.params.fit,offsetX:f=ir.params.offsetX,offsetY:p=ir.params.offsetY,originX:h=ir.params.originX,originY:g=ir.params.originY,rotation:_=ir.params.rotation,scale:v=ir.params.scale,worldHeight:y=ir.params.worldHeight,worldWidth:b=ir.params.worldWidth,...x}){let S=typeof n==`string`?n:n.src,[C,w]=(0,k.useState)(Qn),T;T=u&&typeof window<`u`?rr(()=>Ke(S).then(e=>URL.createObjectURL(e.blob)),[S,`heatmap`]):C,(0,k.useLayoutEffect)(()=>{if(u)return;if(!S){w(Qn);return}let e,t=!0;return Ke(S).then(n=>{t&&(e=URL.createObjectURL(n.blob),w(e))}),()=>{t=!1}},[S,u]);let E=(0,k.useMemo)(()=>({u_image:T,u_contour:r,u_angle:i,u_noise:a,u_innerGlow:o,u_outerGlow:s,u_colorBack:O(c),u_colors:l.map(O),u_colorsCount:l.length,u_fit:m[d],u_offsetX:f,u_offsetY:p,u_originX:h,u_originY:g,u_rotation:_,u_scale:v,u_worldHeight:y,u_worldWidth:b}),[e,t,r,i,a,o,s,l,c,T,d,f,p,h,g,_,v,y,b]);return(0,A.jsx)(j,{...x,speed:e,frame:t,fragmentShader:Ge,mipmaps:[`u_image`],uniforms:E})},M),X={name:`Default`,params:{...f,scale:.6,speed:1,frame:0,colorBack:`#AAAAAC`,colorTint:`#ffffff`,distortion:.07,repetition:2,shiftRed:.3,shiftBlue:.3,contour:.4,softness:.1,angle:70,shape:`diamond`}},sr=[X,{name:`Noir`,params:{...f,scale:.6,speed:1,frame:0,colorBack:`#000000`,colorTint:`#606060`,softness:.45,repetition:1.5,shiftRed:0,shiftBlue:0,distortion:0,contour:0,angle:90,shape:`diamond`}},{name:`Backdrop`,params:{...f,speed:1,frame:0,scale:1,colorBack:`#AAAAAC`,colorTint:`#ffffff`,softness:.05,repetition:1.5,shiftRed:.3,shiftBlue:.3,distortion:.1,contour:.4,shape:`none`,angle:90,worldWidth:0,worldHeight:0}},{name:`Stripes`,params:{...f,speed:1,frame:0,scale:.6,colorBack:`#000000`,colorTint:`#2c5d72`,softness:.8,repetition:6,shiftRed:1,shiftBlue:-1,distortion:.4,contour:.4,shape:`circle`,angle:0}}],cr=(0,k.memo)(function({colorBack:e=X.params.colorBack,colorTint:t=X.params.colorTint,speed:n=X.params.speed,frame:r=X.params.frame,image:i=``,contour:a=X.params.contour,distortion:o=X.params.distortion,softness:s=X.params.softness,repetition:c=X.params.repetition,shiftRed:l=X.params.shiftRed,shiftBlue:u=X.params.shiftBlue,angle:d=X.params.angle,shape:f=X.params.shape,suspendWhenProcessingImage:p=!1,fit:h=X.params.fit,scale:g=X.params.scale,rotation:_=X.params.rotation,originX:v=X.params.originX,originY:y=X.params.originY,offsetX:b=X.params.offsetX,offsetY:x=X.params.offsetY,worldWidth:S=X.params.worldWidth,worldHeight:C=X.params.worldHeight,...w}){let T=typeof i==`string`?i:i.src,[E,ee]=(0,k.useState)(Qn),te;te=p&&typeof window<`u`&&T?rr(()=>Ze(T).then(e=>URL.createObjectURL(e.pngBlob)),[T,`liquid-metal`]):E,(0,k.useLayoutEffect)(()=>{if(p)return;if(!T){ee(Qn);return}let e,t=!0;return Ze(T).then(n=>{t&&(e=URL.createObjectURL(n.pngBlob),ee(e))}),()=>{t=!1}},[T,p]);let ne={u_colorBack:O(e),u_colorTint:O(t),u_image:te,u_contour:a,u_distortion:o,u_softness:s,u_repetition:c,u_shiftRed:l,u_shiftBlue:u,u_angle:d,u_isImage:!!i,u_shape:et[f],u_fit:m[h],u_scale:g,u_rotation:_,u_offsetX:b,u_offsetY:x,u_originX:v,u_originY:y,u_worldWidth:S,u_worldHeight:C};return(0,A.jsx)(j,{...w,speed:n,frame:r,fragmentShader:Ye,mipmaps:[`u_image`],uniforms:ne})}),Z={name:`Default`,params:{...f,fit:`cover`,speed:0,frame:0,colorBack:`#f2f1e8`,colorFront:`#2b2b2b`,size:.5,radius:1.25,contrast:.4,originalColors:!1,inverted:!1,grainMixer:.2,grainOverlay:.2,grainSize:.5,grid:`hex`,type:`gooey`}},lr=[Z,{name:`LED screen`,params:{...f,fit:`cover`,speed:0,frame:0,colorBack:`#000000`,colorFront:`#29ff7b`,size:.5,radius:1.5,contrast:.3,originalColors:!1,inverted:!1,grainMixer:0,grainOverlay:0,grainSize:.5,grid:`square`,type:`soft`}},{name:`Mosaic`,params:{...f,fit:`cover`,speed:0,frame:0,colorBack:`#000000`,colorFront:`#b2aeae`,size:.6,radius:2,contrast:.01,originalColors:!0,inverted:!1,grainMixer:0,grainOverlay:0,grainSize:.5,grid:`hex`,type:`classic`}},{name:`Round and square`,params:{...f,fit:`cover`,speed:0,frame:0,colorBack:`#141414`,colorFront:`#ff8000`,size:.8,radius:1,contrast:1,originalColors:!1,inverted:!0,grainMixer:.05,grainOverlay:.3,grainSize:.5,grid:`square`,type:`holes`}}],ur=(0,k.memo)(function({speed:e=Z.params.speed,frame:t=Z.params.frame,colorFront:n=Z.params.colorFront,colorBack:r=Z.params.colorBack,image:i=``,size:a=Z.params.size,radius:o=Z.params.radius,contrast:s=Z.params.contrast,originalColors:c=Z.params.originalColors,inverted:l=Z.params.inverted,grainMixer:u=Z.params.grainMixer,grainOverlay:d=Z.params.grainOverlay,grainSize:f=Z.params.grainSize,grid:p=Z.params.grid,type:h=Z.params.type,fit:g=Z.params.fit,scale:_=Z.params.scale,rotation:v=Z.params.rotation,originX:y=Z.params.originX,originY:b=Z.params.originY,offsetX:x=Z.params.offsetX,offsetY:S=Z.params.offsetY,worldWidth:C=Z.params.worldWidth,worldHeight:w=Z.params.worldHeight,...T}){let E={u_image:i,u_colorFront:O(n),u_colorBack:O(r),u_size:a,u_radius:o,u_contrast:s,u_originalColors:c,u_inverted:l,u_grainMixer:u,u_grainOverlay:d,u_grainSize:f,u_grid:rt[p],u_type:nt[h],u_fit:m[g],u_rotation:v,u_scale:_,u_offsetX:x,u_offsetY:S,u_originX:y,u_originY:b,u_worldWidth:C,u_worldHeight:w};return(0,A.jsx)(j,{...T,speed:e,frame:t,fragmentShader:tt,uniforms:E})},M),Q={name:`Default`,params:{...f,scale:1,fit:`cover`,speed:0,frame:0,colorBack:`#fbfaf5`,colorC:`#00b4ff`,colorM:`#fc519f`,colorY:`#ffd800`,colorK:`#231f20`,size:.2,contrast:1,softness:1,grainSize:.5,grainMixer:0,grainOverlay:0,gridNoise:.2,floodC:.15,floodM:0,floodY:0,floodK:0,gainC:.3,gainM:0,gainY:.2,gainK:0,type:`ink`}},dr=[Q,{name:`Drops`,params:{...f,scale:1,fit:`cover`,speed:0,frame:0,colorBack:`#eeefd7`,colorC:`#00b2ff`,colorM:`#fc4f4f`,colorY:`#ffd900`,colorK:`#231f20`,size:.88,contrast:1.15,softness:0,grainSize:.01,grainMixer:.05,grainOverlay:.25,gridNoise:.5,floodC:.15,floodM:0,floodY:0,floodK:0,gainC:1,gainM:.44,gainY:-1,gainK:0,type:`ink`}},{name:`Newspaper`,params:{...f,scale:1,fit:`cover`,speed:0,frame:0,colorBack:`#f2f1e8`,colorC:`#7a7a75`,colorM:`#7a7a75`,colorY:`#7a7a75`,colorK:`#231f20`,size:.01,contrast:2,softness:.2,grainSize:0,grainMixer:0,grainOverlay:.2,gridNoise:.6,floodC:0,floodM:0,floodY:0,floodK:.1,gainC:-.17,gainM:-.45,gainY:-.45,gainK:0,type:`dots`}},{name:`Vintage`,params:{...f,scale:1,fit:`cover`,speed:0,frame:0,colorBack:`#fffaf0`,colorC:`#59afc5`,colorM:`#d8697c`,colorY:`#fad85c`,colorK:`#2d2824`,size:.2,contrast:1.25,softness:.4,grainSize:.5,grainMixer:.15,grainOverlay:.1,gridNoise:.45,floodC:.15,floodM:0,floodY:0,floodK:0,gainC:.3,gainM:0,gainY:.2,gainK:0,type:`sharp`}}],fr=(0,k.memo)(function({speed:e=Q.params.speed,frame:t=Q.params.frame,colorBack:n=Q.params.colorBack,colorC:r=Q.params.colorC,colorM:i=Q.params.colorM,colorY:a=Q.params.colorY,colorK:o=Q.params.colorK,image:s=``,size:c=Q.params.size,contrast:l=Q.params.contrast,softness:u=Q.params.softness,grainSize:d=Q.params.grainSize,grainMixer:f=Q.params.grainMixer,grainOverlay:p=Q.params.grainOverlay,gridNoise:h=Q.params.gridNoise,floodC:g=Q.params.floodC,floodM:_=Q.params.floodM,floodY:v=Q.params.floodY,floodK:y=Q.params.floodK,gainC:b=Q.params.gainC,gainM:x=Q.params.gainM,gainY:S=Q.params.gainY,gainK:C=Q.params.gainK,type:w=Q.params.type,fit:T=Q.params.fit,scale:E=Q.params.scale,rotation:ee=Q.params.rotation,originX:te=Q.params.originX,originY:ne=Q.params.originY,offsetX:re=Q.params.offsetX,offsetY:ie=Q.params.offsetY,worldWidth:ae=Q.params.worldWidth,worldHeight:D=Q.params.worldHeight,...oe}){let se={u_image:s,u_noiseTexture:yt(),u_colorBack:O(n),u_colorC:O(r),u_colorM:O(i),u_colorY:O(a),u_colorK:O(o),u_size:c,u_contrast:l,u_softness:u,u_grainSize:d,u_grainMixer:f,u_grainOverlay:p,u_gridNoise:h,u_floodC:g,u_floodM:_,u_floodY:v,u_floodK:y,u_gainC:b,u_gainM:x,u_gainY:S,u_gainK:C,u_type:at[w],u_fit:m[T],u_rotation:ee,u_scale:E,u_offsetX:re,u_offsetY:ie,u_originX:te,u_originY:ne,u_worldWidth:ae,u_worldHeight:D};return(0,A.jsx)(j,{...oe,speed:e,frame:t,fragmentShader:it,uniforms:se})},M),$={name:`Default`,params:{...f,scale:.6,speed:1,frame:0,colorBack:`#f0efea`,colorInner:`#fafaf5`,colors:[`#333333`,`#e7e6df`],outerGlow:.55,innerGlow:1,innerDistortion:.8,outerDistortion:.6,offset:0,angle:0,size:.8,shape:`diamond`}},pr={name:`Fluorescent`,params:{...f,scale:.6,speed:1,frame:0,colorBack:`#000000`,colorInner:`#000000`,colors:[`#2fb64c`,`#cdff61`,`#ffffff`],outerGlow:0,innerGlow:1,innerDistortion:1,outerDistortion:.8,offset:0,angle:0,size:.8,shape:`diamond`}},mr=[$,{name:`Fire`,params:{...f,scale:.6,speed:1,frame:0,colorBack:`#000000`,colorInner:`#000000`,colors:[`#fe5b16`,`#f7ff61`,`#ffffff`],outerGlow:1,innerGlow:.65,innerDistortion:.6,outerDistortion:.8,offset:0,angle:0,size:.8,shape:`diamond`}},pr,{name:`Infrared`,params:{...f,scale:.6,speed:.5,frame:0,colorBack:`#cd28dc`,colorInner:`#00000000`,colors:[`#ff9900`,`#fff67a`,`#dcff52`,`#00ffbb`,`#0077ff`],outerGlow:1,innerGlow:1,innerDistortion:1,outerDistortion:1,offset:.2,angle:0,size:1,shape:`diamond`}}],hr=(0,k.memo)(function({colorBack:e=$.params.colorBack,colors:t=$.params.colors,speed:n=$.params.speed,frame:r=$.params.frame,image:i=``,innerDistortion:a=$.params.innerDistortion,outerDistortion:o=$.params.outerDistortion,outerGlow:s=$.params.outerGlow,innerGlow:c=$.params.innerGlow,colorInner:l=$.params.colorInner,offset:u=$.params.offset,angle:d=$.params.angle,size:f=$.params.size,shape:p=$.params.shape,suspendWhenProcessingImage:h=!1,fit:g=$.params.fit,scale:_=$.params.scale,rotation:v=$.params.rotation,originX:y=$.params.originX,originY:b=$.params.originY,offsetX:x=$.params.offsetX,offsetY:S=$.params.offsetY,worldWidth:C=$.params.worldWidth,worldHeight:w=$.params.worldHeight,...T}){let E=typeof i==`string`?i:i.src,[ee,te]=(0,k.useState)(Qn),ne;ne=h&&typeof window<`u`&&E?rr(()=>lt(E).then(e=>URL.createObjectURL(e.pngBlob)),[E,`gemSmoke`]):ee,(0,k.useLayoutEffect)(()=>{if(h)return;if(!E){te(Qn);return}let e,t=!0;return lt(E).then(n=>{t&&(e=URL.createObjectURL(n.pngBlob),te(e))}),()=>{t=!1}},[E,h]);let re={u_colors:t.map(O),u_colorsCount:t.length,u_colorBack:O(e),u_image:ne,u_innerDistortion:a,u_outerDistortion:o,u_outerGlow:s,u_innerGlow:c,u_colorInner:O(l),u_offset:u,u_angle:d,u_size:f,u_isImage:!!i,u_shape:ft[p],u_fit:m[g],u_scale:_,u_rotation:v,u_offsetX:x,u_offsetY:S,u_originX:y,u_originY:b,u_worldWidth:C,u_worldHeight:w};return(0,A.jsx)(j,{...T,speed:n,frame:r,fragmentShader:st,mipmaps:[`u_image`],uniforms:re})},M);export{kn as ColorPanels,Sn as Dithering,Bt as DotGrid,Rt as DotOrbit,Hn as FlutedGlass,hr as GemSmoke,ln as GodRays,Tn as GrainGradient,fr as HalftoneCmyk,ur as HalftoneDots,or as Heatmap,Yn as ImageDithering,Zn as LensDistortion,cr as LiquidMetal,Ot as MeshGradient,Yt as Metaballs,Pt as NeuroNoise,zn as PaperTexture,en as PerlinNoise,Dn as PulsingBorder,j as ShaderMount,Wt as SimplexNoise,jt as SmokeRing,fn as Spiral,Mn as StaticMeshGradient,In as StaticRadialGradient,hn as Swirl,an as Voronoi,sn as Warp,Kn as Water,Qt as Waves,Ae as colorPanelsMeta,On as colorPanelsPresets,xn as ditheringPresets,zt as dotGridPresets,ne as dotOrbitMeta,Lt as dotOrbitPresets,Vn as flutedGlassPresets,mr as gemSmokePresets,O as getShaderColorFromString,ge as godRaysMeta,cn as godRaysPresets,we as grainGradientMeta,wn as grainGradientPresets,dr as halftoneCmykPresets,lr as halftoneDotsPresets,We as heatmapMeta,ar as heatmapPresets,Jn as imageDitheringPresets,l as isPaperShaderElement,He as lensDistortionMeta,Xn as lensDistortionPresets,sr as liquidMetalPresets,w as meshGradientMeta,Dt as meshGradientPresets,se as metaballsMeta,Jt as metaballsPresets,Nt as neuroNoisePresets,Rn as paperTexturePresets,$t as perlinNoisePresets,De as pulsingBorderMeta,En as pulsingBorderPresets,D as simplexNoiseMeta,Ut as simplexNoisePresets,E as smokeRingMeta,At as smokeRingPresets,dn as spiralPresets,Me as staticMeshGradientMeta,jn as staticMeshGradientPresets,Pe as staticRadialGradientMeta,Fn as staticRadialGradientPresets,ye as swirlMeta,mn as swirlPresets,ue as voronoiMeta,rn as voronoiPresets,pe as warpMeta,on as warpPresets,Gn as waterPresets,Zt as wavesPresets};